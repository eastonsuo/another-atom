...
function ProjectChatPanel({ run, events, refreshShell, refreshRun, setError, visionEnabled, language, onReviseProductSpec }: { run: RunView; events: RunEvent[]; refreshShell: () => Promise<void>; refreshRun: (id: string) => Promise<RunView>; setError: (value: string) => void; visionEnabled: boolean; language: Language; onReviseProductSpec: (instruction: string) => Promise<void> }) {
  ...
  const imageAttachments = useImageAttachments(language);
  const retrySubmissionRef = useRef<{ signature: string; clientMessageId: string } | null>(null);
  ...
  const submitChange = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const submissionSignature = JSON.stringify({
      ...
      attachmentIds: imageAttachments.readyIds,
    });
    const optimisticMessageId = retrySubmissionRef.current?.signature === submissionSignature
      ? retrySubmissionRef.current.clientMessageId
    ...
    if (optimisticMessage) {
      setProjectMessages((current) => current.some(
        (message) => message.id === optimisticMessageId
          || message.payload.client_message_id === optimisticMessageId,
    ...
    try {
      ...
      try {
        const persistedMessages = await api.projectMessages(run.project_id);
        setProjectMessages(persistedMessages);
        setMessagesError("");
      } catch (refreshReason) {
        setMessagesError(refreshReason instanceof Error ? refreshReason.message : ui(language, "Could not load Project messages"));
      }
    } finally {
...
function ProjectChatMessageRow({ message, language, roleName, approvingProposalId, onApproveProposal }: { message: ProjectMessageView; language: Language; roleName: (role: ProjectMessageView["role"]) => string; approvingProposalId: string; onApproveProposal: (proposal: ProjectMessageView) => Promise<void> }) {
  ...
  const failureReason = typeof message.payload.reason === "string" ? message.payload.reason : "";
  ...
  const displayedOutput = streaming ? animatedOutput : modelOutput || failureReason;
  useEffect(() => {
  ...
  const showModelOutput = (message.message_type === "agent_update" || message.message_type === "error" || Boolean(modelOutput)) && (streaming || Boolean(modelOutput) || status === "failed");
