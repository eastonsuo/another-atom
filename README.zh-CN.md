...
| 版本 | 目标 | 角色模式 | 当前状态 |
| --- | --- | --- | --- |
| **V1** | 交付一条完整、可公开测试的应用生成链路 | Team Leader、Product Manager、Architect、Engineer、Data Analyst 固定顺序接力 | 本地纵切已实现，继续加固并部署云端 |
| **V2** | 增加自主协作、动态分派、返工和仲裁 | Team Leader 协调 Product Manager、Architect、Engineer、Data Analyst | V1 完成后实施 |
...
4. Architect 生成 **ArchitectureSpec**：定义路由、数据边界、视觉 token 和交互结构。
5. Engineer 生成 **AppSpec**，并负责确定性构建与交互校验。
6. Data Analyst 检查商品数据完整性，并解释不可变的工程校验证据。
...
- **怎么往下走：** 只有用户确认后，Architect、Engineer、Data Analyst 才会依次产出 ArchitectureSpec、AppSpec 和 DataReview。没有确认或没有真实产物，阶段不能标记为完成。
...
| **M2 生成链路** | Prompt、附件元数据、Blueprint 审批、固定角色顺序、Run 级模型选择和持久化 Build Job | 未确认 Blueprint 不构建；每阶段有 Schema 产物；失败状态可见 | Ollama Cloud + Mock 已实现 |
...
Team Leader、Product Manager、Architect、Engineer、Data Analyst 的意义不在于展示多个角色名称，而在于逐步收敛不同类型的不确定性：

```text
协调层  Run 状态             -> StageDecision       控制顺序、重试和失败收敛
...
每个专业角色交接产物都要通过 Schema 校验、持久化并在界面中可检查。V1 Team Leader 由确定性 Orchestrator 实现，只产生可审计的 StageDecision/进度事件，不伪造模型讨论。
...
| 创新性 | Blueprint/ArchitectureSpec/AppSpec 产物链、受控角色接力和版本化发布闭环 |
