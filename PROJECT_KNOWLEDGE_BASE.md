...
Another Atom 是一个以 Project 为中心的多智能体 Vibe Coding 工作台：用户通过自然语言提出任意软件产品目标，Agent 负责需求整理、技术结构、代码生成和质量解释；用户可以查看和修改源码、管理版本，并自主决定何时发布。网页项目还可以在 Studio 中直接预览和进行视觉修改。
...
产品范围不由 Preview 决定。系统必须保留用户要求的项目类型和目标平台：网页项目可以使用当前浏览器 Preview；非 Web 项目仍进入同一个 Project、代码仓库和版本体系，但只有在存在对应 Runtime 适配器时才能直接构建或运行。当前代码已经完成 Web 源码与浏览器 Preview，非 Web 源码 Contract、构建和运行适配仍是实现缺口，不能描述为已完成，也不能通过改写成 Web 项目绕过。
...
**[当前 Project Chat 纵切]** 已有 Project 消息使用独立 `ProjectLeadDecision(answer|clarify|propose_change)`。Runtime 在每次调用前组装当前 ProductSpec、Blueprint、ArchitectureSpec、AppSpec 元数据、Project 对话、失败摘要，以及按 `MAX_SOURCE_CHARS` 从基线 commit 装箱的实际源码；Provider 接收完整 Context，ProjectMessage 只保存 hash 与 manifest。answer/clarify 不建 Run；propose_change 先保存任务卡，点击“修改代码”后才建 Run、BuildJob 和写占用。当前路由仍是同步 HTTP 调用，尚无 ConversationJob/lease、消息幂等键和通用 Approval subject。
...
- Lead direct/team 路由；
- 已有 Project 的 answer/clarify/propose_change 路由、Context hash、修改任务卡和批准后建 Run；
...
- 可重启恢复的异步 Project ConversationJob、消息幂等键和统一 Memory；
...
- `README.md`：产品入口和整体架构。
- `docs/design/整体/01-[产品]-整体产品目标与定位.md`：跨版本产品决策。
- `docs/design/整体/02-[参考]-Atoms参考产品分析.md`：Atoms 参考。
...
- `docs/design/V2/产品设计/01-产品范围与交互.md`：V2 产品范围与验收。
- `docs/design/V2/技术设计/01-[Agent]-任务编排与多Agent协作.md`：Leader、TaskGraph、Handoff、Tool 和收敛。
- `docs/design/V2/技术设计/02-[工程]-多Agent执行与沙箱架构.md`：Worker、PostgreSQL Lease、Object Storage 和 Sandbox。
