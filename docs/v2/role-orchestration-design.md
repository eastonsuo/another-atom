...
| Planner Agent | Product Agent | 职责从需求结构化扩展到澄清和范围管理 |
...
| Product | 用户到底要什么，是否在支持范围 | Prompt、附件元数据、用户反馈 | `Blueprint`、`ClarificationRequest` |
...
- 不替 Product、Architect、Designer、Engineer、QA 生成专业产物。
...
sequenceDiagram
    ...
    Leader-->>Runtime: Product first
    Runtime->>Product: analyze requirement
...
| `requirements` 或 `scope` | Product，必要时请求用户 |
...
| Product | 无 Shell/文件工具；可读取用户输入和 Blueprint 历史 |
