...
Team Leader(runtime) -> Product Manager -> Architect -> Engineer -> Data Analyst
...
V2 采用 **1 个 Team Leader + 4 个专业 Agent**，与参考产品的核心角色口径一致。Architect 同时负责技术结构、数据契约、视觉 token 和交互边界；Engineer 内聚代码级测试，Data Analyst 内聚数据处理、工具分析与验证证据解释，不设置独立 Designer 或 QA 角色。
...
| Agent 拓扑 | Team Leader Runtime + Product Manager、Architect、Engineer、Data Analyst 固定顺序 | Team Leader 升级为独立 Agent，形成 1 Leader + 4 Specialist Agents |
...
| Sandbox | 共享服务内的固定模板受控构建，不运行任意代码 | Engineer 任务进入 Run 级隔离环境；Data Analyst 使用同一快照的只读检查环境 |
...
Blueprint 由用户确认后，Architect 可以与 Data Analyst 的数据准备任务局部并行；Engineer 必须等待 ArchitectureSpec 和所需 DataProfile 都通过 Contract。
...
- V1 以确定性 Team Leader 阶段展示编排；V2 Leader 在实际规划和仲裁时作为 Agent Coordinator 可见。
- V2 采用 Team Leader、Product Manager、Architect、Engineer、Data Analyst，形成 1 Leader + 4 Specialist Agents。
