...
V2 在 V1 可部署闭环上实现**受 Runtime 约束的自主多 Agent 协作**：V1 已有的 Lead 从 `direct/team` 二选一路由升级为 TaskGraph 协调者；Product Manager、Architect、Engineer、Data Analyst 使用独立 Context 完成任务，通过 Handoff 交付；Engineer/Data Analyst 可以在 Run 级 Sandbox 内申请受控 Tool，失败时依据 Evidence 进行有预算的返工与收敛。

```text
V1: Lead 二选一路由 + 固定团队
User -> Lead -> direct | Product Manager -> Architect -> Engineer -> Data Analyst

V2: Runtime 约束的自主多 Agent
User -> Lead -> direct | TaskGraph
                         |
             +----------------+----------------+
             v                v                v
...
- Lead 判断为 `team` 后，所有请求都按同一完整顺序执行，无法根据任务复杂度调整角色和依赖。
- Agent 只产生结构化 Contract，没有文件、构建、测试和浏览器 Tool，无法处理更真实的实现与修复任务。
- 自动修复最多一轮，只能回到 Engineer，无法把规格、视觉或需求根因路由给责任角色。
- 所有角色按顺序运行，架构设计与独立数据准备等无依赖任务也不能并行。
- V1 只有阶段级 Context，无法验证独立 Agent 的 Handoff、拒收、仲裁和预算治理。

V2 需要增加自主性，但必须保留 V1 已证明有效的风险驱动确认、Artifact Contract、用户级隔离、Project 本地 Git、确定性 Validator、版本历史和用户显式 Publish。
...
3. **用户门禁继承 V1**：普通 supported 任务不重复审批；adapted/范围变更、越权 Tool、预算追加、破坏性操作和 Publish 必须由用户确认。
4. **确定性验收优先**：Data Analyst Agent 不能覆盖 Build/Test/Validation 的 mandatory 结果。
5. **有预算地收敛**：每个 Run 有调用、token、时间、并发和 Artifact 修订上限。
6. **V1 可回归**：V2 的数据、事件和 UI 扩展不能破坏 V1 ProjectVersion、Publish 和公开 URL 语义。

## 6. P0 功能范围

### 6.1 Autonomous Team Execution

- 用户仍只面对 Lead，不新增必须理解的模式选择器。Lead 可以 direct，也可以提出 team TaskGraph；用户可显式要求“调用团队”。
- Product Manager 先产生 Blueprint。普通 supported 需求可进入 TaskGraph；adapted、范围变化或预算超出基础额度时先触发 Risk Policy。
- Leader 基于当前有效 Blueprint 生成 TaskGraph，Runtime 校验依赖、角色、预算和允许的并行组。
...
- Architect 统一负责 ArchitectureSpec、数据 Contract、视觉 token、InteractionSpec 和可行性判断。
- Engineer 在 Sandbox 内执行 ImplementationPlan、文件修改、Build 和 Test Tool。
- Data Analyst 基于只读快照和确定性 Evidence 设计检查、解释失败并提出 ReworkRequest。
...
- Data Analyst P0 Tool：`inspect_artifact`、`run_tests`、`browser_check`、`capture_screenshot`，默认只读。
- 每个 Engineer Task 使用独立可写快照；Data Analyst 使用对应快照的只读副本；多个 Agent 不能并发写同一目录。
- 超出 allowlist 的依赖、网络或写入范围必须等待用户确认。

### 6.5 选择性并行

- Runtime 只并行执行 TaskGraph 中无依赖冲突、无共享写入且预算已预占的节点。
- 第一条并行路径限定为 Architect 与 Data Analyst 中已被 Runtime 判定无依赖、无共享写入的架构/数据准备子任务。
- UI 显示并行分支、各自状态、消耗和 Handoff；不能用同时闪动角色头像伪装并行。
- 任一分支失败不自动回滚已成功分支的 LLM 用量或 Artifact；失败分支按策略重试、降级或请求用户。

### 6.6 Rework、Arbitration 与收敛

- Data Analyst/下游角色基于 Evidence 生成 ReworkRequest，Runtime 按 root cause 路由给责任角色。
...
### 6.8 Data Analyst、版本与发布

- mandatory Build/Test/Validation Evidence 仍由确定性系统产生，Data Analyst Agent 不能改写。
- DataReview 提供用户可读摘要、Evidence 链接、warning 和 Rework 建议。
...
1. 用户向 Lead 输入 Mono Market Prompt；Lead 路由到 team，Product Manager 生成 supported Blueprint。
2. Leader 提交 TaskGraph，Runtime 校验后展示任务与预算。
3. Architect 与 Data Analyst 的数据准备任务在允许条件下并行，分别交付 ArchitectureSpec 与 DataProfile。
4. Engineer 接收已接受的 Handoff，在 Sandbox 内修改、构建和测试。
5. Data Analyst 在只读快照上检查并交付 DataReview。
6. mandatory checks 通过后生成 ProjectVersion，用户预览并显式 Publish。

### 7.2 返工路径

1. Data Analyst Evidence 指向 implementation failure，生成 ReworkRequest。
2. Runtime 校验 root cause 和预算，路由给 Engineer。
3. Engineer 基于原 Artifact 新建修订，重新 Build/Test。
4. Data Analyst 复验；成功则合并，达到预算则停止并请求用户。

### 7.3 Human-in-the-loop 路径

出现以下任一情况时暂停：adapted/范围变化、越权依赖或网络、追加预算、无法仲裁冲突、破坏性仓库操作、Restore/Unpublish/Publish。用户批准只绑定精确 Artifact/ToolRequest/Budget version；对象变化后旧批准失效。
...
```text
Run
Created -> LeadRouting -> Planning -> Running
                                               |       |
                                               |       +-> AwaitingRiskApproval
                                               |       +-> Reworking
                                               |       +-> PartiallyFailed
                                               v
...
- Lead 的 autonomous team 路径真实生成并持久化 TaskGraph、Task、Artifact、Handoff、ToolRequest、Evidence 和 RunBudget。
- 至少一条无依赖分支真实并行执行，并能从事件时间与独立 Task 状态验证。
- 至少一条 Evidence 驱动 Rework 路径能路由、修订、重建、复验并收敛。
- adapted/范围变化、越权 Tool、预算追加、破坏性仓库操作和 Publish 的 Human-in-the-loop 均不能被 Agent 绕过。
- mandatory failure 不能被 DataReview 或 LeaderDecision 改写为通过。
...
- Data Analyst 是否在 P0 使用视觉截图评估，以及对应测试集。
- V2 是否扩展商品目录站以外的应用类型。
