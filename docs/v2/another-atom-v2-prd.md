...
V2 在 V1 可部署闭环上实现**受 Runtime 约束的自主多 Agent 协作**：Team Leader 根据用户目标和现有 Artifact 提出 TaskGraph，Product Manager、Architect、Engineer、Data Analyst 使用独立 Context 完成任务，通过 Handoff 交付；Engineer/Data Analyst 可以在 Run 级 Sandbox 内申请受控 Tool，失败时依据 Evidence 进行有预算的返工与收敛。
