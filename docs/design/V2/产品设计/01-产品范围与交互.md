# Another Atom V2 产品范围与交互设计

[toc]

- 文档状态：V2 产品设计基线草案；V1 验收完成后进入开发
- 更新日期：2026-07-11
- 整体产品：[Another Atom 整体产品目标与定位](../../整体/01-[产品]-整体产品目标与定位.md)
- V1 产品基线：[Another Atom V1 核心产品需求与交互](../../V1/产品设计/01-核心产品需求与交互.md)
- V2 Agent 设计：[Another Atom V2 任务编排与多 Agent 协作](../技术设计/01-[Agent]-任务编排与多Agent协作.md)
- V2 工程设计：[Another Atom V2 多 Agent 执行与沙箱架构](../技术设计/02-[工程]-多Agent执行与沙箱架构.md)

## 背景

V1 用固定团队证明端到端闭环后，`direct/team` 二选一路由和固定顺序无法覆盖需要动态拆解、角色子集、局部并行或多轮返工的复杂任务。V2 在不破坏现有 Project、Artifact、权限和发布边界的前提下扩展协作能力。

## 摘要

- **编排升级**
  - Lead 升级为受 Runtime 约束的 TaskGraph 协调者，专业 Agent 使用独立 Context 和结构化 Handoff 协作。
- **协作能力**
  - TaskGraph 可以按任务选择角色子集、局部并行，并基于 Evidence 发起返工与仲裁。
- **Tool 与 Sandbox**
  - 专业 Agent 可以在角色权限、预算和任务级 Sandbox 内申请受控 Tool。
- **平台控制**
  - 用户审批、身份归属、配额、轮次上限、确定性校验和 Publish 仍由平台控制。
- **版本状态**
  - V2 是 V1 验收后的计划版本，不是当前已实现能力。

## 1. 版本结论

V2 在 V1 可部署闭环上实现**受 Runtime 约束的自主多 Agent 协作**：V1 已有的 Lead 从 `direct/team` 二选一路由升级为 TaskGraph 协调者；Product Manager、Architect、Engineer、Data Analyst、Reviewer 使用独立 Context 完成任务，通过 Handoff 交付；Engineer/Data Analyst/Reviewer 可以在 Run 级 Sandbox 内申请受控 Tool，失败时依据 Evidence 进行有预算的返工与收敛。

```text
V1: Lead 二选一路由 + 固定团队
User -> Lead -> direct | Product Manager -> Architect -> Engineer -> Data Analyst -> Validator -> Reviewer
...
- 不把“接受任意软件产品目标”夸大为任意代码都能直接执行。V2 延续按项目类型选择 Source/Build/Runtime Adapter 的边界；没有适配器的项目仍可保留目标和源码，但不能声称已经可运行或可预览。
...
- 需要把产品需求变成可持续管理的软件项目，并希望检查规划、设计、实现和测试依据的用户；网页项目可额外使用浏览器 Preview。
...
4. **确定性验收优先**：Reviewer 不能覆盖 Build/Test/Validation 的 mandatory 结果；Data Analyst 不承担最终验收职责。
...
- Data Analyst 基于只读快照整理数据、内容与运行状态，产出 DataProfile。
- Reviewer 基于需求、架构、实现、DataProfile 和确定性 Evidence 独立审查，并提出 ReviewReport 或 ReworkRequest。
...
- Reviewer P0 Tool：`inspect_artifact`、`browser_check`、`capture_screenshot`，默认只读，不执行修复。
...
- Reviewer 或下游接收方基于 Evidence 生成 ReworkRequest，Runtime 按 root cause 路由给责任角色。
...
- mandatory Build/Test/Validation Evidence 仍由确定性系统产生，Reviewer Agent 不能改写。
- DataProfile 描述应用数据事实；ReviewReport 提供用户可读结论、Evidence 链接、warning 和 Rework 建议。
...
5. 确定性 Validator 生成 ValidationEvidence，Reviewer 在只读快照上独立复核并交付 ReviewReport。
6. mandatory checks 通过且 Reviewer 接受后生成 ProjectVersion，用户预览并显式 Publish。

### 7.2 返工路径

1. Reviewer 引用 ValidationEvidence 指向 implementation failure，生成 ReworkRequest。
2. Runtime 校验 root cause 和预算，路由给 Engineer。
3. Engineer 基于原 Artifact 新建修订，重新 Build/Test。
4. Validator 复验，Reviewer 再次审查；成功则合并，达到预算则停止并请求用户。
...
- 不把 V2 P0 描述为已经具备全部 Build/Runtime Adapter 的通用代码执行平台。
...
- mandatory failure 不能被 ReviewReport 或 LeaderDecision 改写为通过。
...
- V2 P0 首批新增哪些非 Web Source/Build/Runtime Adapter，以及各自的安全与验收边界。
