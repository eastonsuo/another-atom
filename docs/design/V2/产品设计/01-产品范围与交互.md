...
V2 把 Lead 升级为受 Runtime 约束的 TaskGraph 协调者，专业 Agent 使用独立 Context 和结构化 Handoff 协作，并可在权限与预算内申请 Tool、并行和返工。用户审批、身份归属、配额、轮次上限、确定性校验和 Publish 仍由平台控制；V2 是 V1 验收后的计划版本，不是当前已实现能力。
