...
- 更新日期：2026-07-11
- V2 产品范围：[V2 产品需求文档](../产品设计/产品需求.md)
- V2 工程架构：[V2 架构设计](../工程设计/架构设计.md)
- V1 基线：[V1 架构设计](../../V1/工程设计/架构设计.md)
- V1 Agent 基线：[V1 Agent 设计](../../V1/Agent设计/Agent设计.md)
- V1 产品范围：[V1 产品需求文档](../../V1/产品设计/产品需求.md)
- 参考分析：[Atoms 参考产品功能分析](../../整体/参考资料/Atoms参考产品分析.md)
...
Lead Agent -> direct | Product Manager -> Architect -> Engineer -> Data Analyst
Lead 只做二选一路由；team 内固定顺序，不动态委派、不并行、不协商

V2: Autonomous Multi-Agent System
Team Leader -> Product Manager / Architect / Engineer / Data Analyst
动态委派、局部并行、结构化回退、仲裁与收敛
```

V2 延续 **1 个 Lead + 4 个专业 Agent**。变化不是“新增 Lead”，而是把 V1 仅能 `direct/team` 二选一的 Lead 升级为 TaskGraph 协调者。Architect 同时负责技术结构、数据契约、视觉 token 和交互边界；Engineer 内聚代码级测试，Data Analyst 内聚数据处理、工具分析与验证证据解释，不设置独立 Designer 或 QA 角色。
...
- Engineer、Data Analyst 只有在任务需要文件、构建、测试或浏览器证据时，才进入有预算、有轮次上限的 bounded ReAct。
...
| Agent 拓扑 | 独立 Lead Agent 二选一路由；team 内四个 Specialist 固定顺序 | Lead 升级为 TaskGraph 协调者；四个 Specialist 可按图执行 |
...
| 验收 | Validator 决定 mandatory pass/fail，Data Analyst 解释 | 保留确定性门禁；Data Analyst 增加测试规划和根因请求，但不能覆盖 Evidence |
...
| Data Analyst Agent + Validator | Data Analyst Agent + Validator | 保留确定性门禁，增加测试规划和根因判断 |
...
| Data Analyst | 数据是否完整，结果证据说明什么 | Blueprint/AppSpec、BuildArtifact、确定性 ValidationEvidence | `DataProfile`、`DataReview`、`ReworkRequest` |
...
- 不替 Product Manager、Architect、Engineer、Data Analyst 生成专业产物。
...
输入：Blueprint、AppSpec、BuildArtifact、ArchitectureSpec、InteractionSpec 和平台生成的 `ValidationEvidence`。

输出：构建前可产生 `DataProfile`，构建后产生 `DataReview` 或 `ReworkRequest`。
...
Data Analyst Agent 可以执行结构化数据处理、分析产品数据完整性、解释验证证据并建议回退；不能修改 Specs，也不能覆盖 Engineer/Validator 的事实结果。

## 7. 发布门与 Data Analyst 分数
...
sequenceDiagram
    ...
    Runtime->>Data Analyst: artifacts + ValidationEvidence
    Data Analyst-->>Runtime: DataReview recommendation=accept
...
如果 Data Analyst 只能怀疑根因，标记 `root_cause=unknown`，由 Leader 基于 Artifacts 建议目标；Runtime 不允许 Data Analyst 绕过 Leader 直接触发 Engineer 或 Architect。
...
- 预算预占、结算、释放和部分失败补偿由 Runtime 执行，具体事务语义以 [V2 架构设计：并行配额与部分失败](../工程设计/架构设计.md#5-并行配额与部分失败)为准。
...
- Data Analyst 获取待验收快照的只读副本，不能修改 Engineer 产物。
...
- 同一时刻只高亮一个角色。
- 每个角色展示 Blueprint、ArchitectureSpec、AppSpec、ValidationReport/DataReview 入口。
...
1. 冻结并版本化 V1 Blueprint、ArchitectureSpec、AppSpec、DataReview Contract。
...
5. 为 Engineer 和 Data Analyst 建立独立沙箱与只读/写入 Tool Policy。
...
- V2 采用 Lead、Product Manager、Architect、Engineer、Data Analyst，形成 1 Lead + 4 Specialist Agents。
- Data Analyst 发布门以 deterministic mandatory checks 和 blocker/error 为准，不预设无依据数字阈值。
...
- 远程沙箱实现、网络策略和依赖白名单。
- Data Analyst 是否加入页面数据可视化截图分析，以及对应测试集。
