...
| 身份与租户 | 沿用 V1 用户名密码 Session Gateway；V1 tenant=User | Task、Artifact、Tool 和 Sandbox 查询继续绑定登录用户；V2 P0 不新增 Organization |
| 源码事实 | 沿用每 Project 一个服务端本地 Git 仓库 | Task Patch 只有合并、构建和验证通过后才由 Repository Service 创建 ProjectVersion commit |
...
| Sandbox | 在 V1 rootless Linux Sandbox Host 上抽象 `SandboxProvider`，按 Task 提供容器、MicroVM 或远程 Sandbox | V1 固定 Vim/Build 边界继续有效；开放更多 Tool 前通过 ADR 和隔离测试 |
| 前端事件 | REST + SSE + OpenAPI | 沿用 V1 `Last-Event-ID` 恢复；增加 agent/handoff/tool/rework/arbitration/budget 事件 |
| 部署 | Control Plane（可在 Railway）+ Agent Worker + PostgreSQL + Object Storage + Linux Sandbox Provider | Web 与 CPU/内存尖峰型 Agent/Build 隔离；Sandbox 不与 Web 共享宿主权限 |
...
4. **隔离先于权限**：Sandbox 未达到要求时，不通过功能开关逐项放宽 V1 的固定 Vim/Build 权限边界。
5. **V1 Contract 向后兼容**：AuthSession、Project、ProjectRepository、Version/commit、Publish、Risk Approval 和基础事件继续可读。
...
User Browser
     ...
     | REST + SSE + WSS
     v
Control Plane (Railway or Linux VM)
React Studio + FastAPI + Session Gateway + Repository API
     |
     +-------- PostgreSQL -----------------------------------+
     ...
     +----> Orchestrator Runtime                              |
       ...
       Agent Worker Service                                  |
                ...
                +---- Tool Gateway                           |
                   ...
                   isolated Git-derived Task workspace       |
                          |                                  |
                          v                                  |
              Build / Test / Browser Evidence                |
                 ...
                 Artifact / Patch / Evidence / Published App
                          |
                          v
                 Repository Service -> Project local Git commit
```

V2 不在 Web Service 内执行 Agent、Build 或 Browser Tool。Web 只处理短请求、SSE/WSS 和状态查询；Agent Worker 负责模型调用和 Task 生命周期；Sandbox 执行不可信程度更高的文件与构建操作。V1 xterm.js/Vim 仍只连接 Project EditorSession，不直接进入 Agent Task Sandbox。
...
- 不挂载 trusted bare repository 或 `.git`；合并后的 allowlist 文件由 Repository Service 提交。
...
- Data Analyst 和 Reviewer 的只读快照不能申请写 Tool。
- Sandbox 销毁失败是平台安全错误，阻止同 Worker 继续领取新任务并触发告警。

### 7.3 Provider 升级决策门

V1 已要求 rootless Linux 容器提供固定 Vim 与固定 Build 隔离。V2 在开放更广 Tool、任务级依赖或受控网络前，必须通过 ADR 决定继续使用强化容器、改用 MicroVM，或接远程 Sandbox Provider，并验证启动延迟、隔离级别、网络策略、资源限制、文件快照、日志脱敏和销毁可靠性。未完成 ADR 和隔离测试时，V2 只能复用 V1 Tool allowlist，不能扩大执行权限。
...
V2 复用 V1 users/auth_sessions/projects/project_repositories/editor_sessions/sessions/artifacts/approvals/versions/deployments/quota/ledger，并新增：
...
- V1 ProjectVersion、`git_commit_sha` 和 Deployment schema 保持向后兼容；合并成功后先提交 Project Git，再创建对应版本记录。
...
7. 上线 Engineer/Data Analyst/Reviewer Tool，再开启单条 Architect/Data Analyst 数据准备并行路径。
...
- mandatory Evidence 不能被 Leader/Reviewer 改写；ProjectVersion 只引用通过门禁且 Reviewer 接受的 Artifact。
- V1 Version/Publish/公开 URL 回归通过，V2 不自动 Publish。
- V2 Golden Path 5/5，通过重启、并行部分失败、预算耗尽和 SSE 重连测试。

## 17. V2 工程演进取舍摘要

本节承接原 README 中的 V2 工程版本说明；组件协议、事务边界和验收条件以前文为准。

- **[服务拆分]** Web/Control Plane 处理身份、命令、状态和事件；Agent Worker 处理模型与 Task 生命周期；Sandbox Provider 执行文件和构建 Tool。
- **[持久化调度]** V2 初期使用 PostgreSQL Task/Lease 与独立 Worker Service，不默认增加消息队列；Task、预算和幂等保持在明确事务边界内。
- **[共享 Artifact Storage]** S3-compatible Object Storage 保存多 Worker 共享的不可变输入、Patch、BuildArtifact、Evidence 和发布快照，不依赖单实例 Volume。
- **[任务级 Sandbox]** Engineer Task 使用独立可写快照，Data Analyst 和 Reviewer 使用相应只读视图；强化容器、MicroVM 或远程 Provider 由 ADR 与隔离测试决定。
- **[并行预算]** 父 RunBudget 在 ready group 启动前原子预留子任务总额度；部分失败保留已接受 Artifact，释放未启动和未使用预算。
- **[多 Worker 恢复]** Task lease、attempt、idempotency key 和 Evidence 防止重复副作用；Web、Agent Worker 与 Sandbox 使用不同服务身份和最小权限。
- **[核心取舍]** V2 接受配额事务、部分失败、资源峰值和部署成本上升，以换取可验证的局部并行；扩展并行度必须以压测和实际成本为依据。
