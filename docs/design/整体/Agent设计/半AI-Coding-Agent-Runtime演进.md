# 半 AI Coding 场景下的 Agent Runtime 设计

[toc]

- 文档性质：基于 Another Atom 当前代码的 Agent 系统学习与演进说明
- 当前实现基线：V1，2026-07-12
- 关联文档：[V1 Agent 设计](../../V1/Agent设计/Agent设计.md) · [V1 架构设计](../../V1/工程设计/架构设计.md) · [V1 实现 Review](../../../review/V1/综合评审/2026-07-12-关键设计与实现检查.md) · [V2 Agent 设计](../../V2/Agent设计/Agent设计.md)

## 1. 核心判断：这不是聊天，也不是完整 Coding Agent

Another Atom 当前处于一种“半 AI Coding”形态：用户不是让 Agent 接管一个已有仓库完成任意工程任务，而是描述一个产品目标，系统把它收敛成可预览、可修改、可版本化、可发布的浏览器应用。
...
一次性的网页生成只需要返回一份代码。半 AI Coding 产品还要处理：用户编辑、旧版本恢复、失败后继续、再次调用 Agent、线上版本不被新草稿覆盖。
...
Lead 与团队 Run 分开：Lead 先做 `direct/team` 二选一路由；进入 team 后，平台按固定状态机调度 Product Manager、Architect、Engineer、Validator、Data Analyst。
...
半 AI Coding 的 Context 超限时，不应平均压缩所有内容。建议固定优先级：
...
| Artifact | Blueprint 到 DataReview 的结构化结果 | 按阶段显式引用 |
...
半 AI Coding 的主要风险不是“记不住”，而是“记住了已经失效的设计”。每条可复用 Memory 至少需要：
...
当前 V1 的固定 Pipeline 并不落后，它适合先证明半 AI Coding 的最小闭环：需求被正确收敛，生成代码可以检查，用户修改不会破坏历史，发布仍由用户决定。
