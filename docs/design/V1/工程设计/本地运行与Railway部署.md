...
- Lead、Product Manager、Architect、Engineer、Data Analyst、Reviewer 固定顺序 Pipeline，Data Analyst 与 Reviewer 之间运行确定性 Validator；
...
4. 在左侧查看 Lead、Product Manager、Architect、Engineer、Data Analyst、Validator、Reviewer 的真实阶段事件。
...
- 默认 Provider 是 Mock；配置 Ollama/DeepSeek 后可运行真实模型，但仍需用完整五角色链路验收模型质量。
