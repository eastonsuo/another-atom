...
**职责：** Validator 不是 LLM 角色。它读取 Prompt、Blueprint、ArchitectureSpec 和 AppSpec，检查范围映射、页面交接、源码完整性、离线边界、视觉 Token 和颜色对比度。`passed=false` 时构建停止，Reviewer 无权覆盖。
...
```text
Input
  AppSpec + DataProfile + ValidationReport + ReviewReport + repair_attempt=0
        |
        v
Engineer Agent
        |
