...
用户能检查 Blueprint、ArchitectureSpec、AppSpec、ValidationReport、DataReview、运行事件和 Project 文件；这些产物不是 README 中的概念展示，而是 V1 状态、恢复和验收的依据。
...
- **[真实 Provider 全链路]** 用真实模型分别验证受控范围内的 supported/adapted/unsupported 判定，并跑通 Product Manager → Architect → Engineer → Data Analyst 的完整链路，核对每阶段 Artifact 和 Usage Ledger。
