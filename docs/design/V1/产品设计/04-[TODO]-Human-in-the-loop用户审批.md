...
用户明确要求“创建这个应用”时，已经授权 Lead 和 PM 整理产品方案，但在用户批准 ProductSpec 前还没有授权进入 Architect 和 Engineer。ProductSpec 批准后，用户明确要求的正常范围和基础预算内执行不再重复确认：
...
后续在这些边界内再次显示“是否允许 Agent 工作”没有新增决策价值，因此自动继续；新的产品基线、范围变化或其他高影响 subject 再调用通用 Approval。
