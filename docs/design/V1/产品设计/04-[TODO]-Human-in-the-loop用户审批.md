...
用户要求生成“使用大模型完成翻译的网页版软件”时，核心产品目标仍是翻译工具，不能改成翻译服务介绍页、语言目录或只有静态文案的展示站。但 V1 生成结果运行在自包含 HTML/CSS/JavaScript 中，不具备服务端 Secret、外部模型 API 和真实模型调用能力，因此不能直接承诺可用的 AI 翻译质量。

Product Manager 必须在 Blueprint 阶段把结果分成两个互斥方案：

- **[接受 adapted 原型]** 保留文本输入、源语言和目标语言选择、翻译触发、加载/失败状态、结果复制等产品交互；翻译结果只能使用明确标注的模拟数据。Approval 卡片必须说明没有真实模型调用、API Key、服务端配额和生产翻译质量。
- **[坚持真实翻译能力]** 如果用户的验收标准是实际调用大模型并评价翻译质量，当前 V1 不能满足，原 Run 进入 Needs input 或停止，不生成一个伪装可用的翻译器。

用户批准 adapted 方案后，验收对象是“翻译软件的产品结构和交互原型是否符合预期”，不是模拟译文的语言质量。生成后对布局、交互和工作流不满意，可以继续在 Project 对话中增量修改；对“为什么翻译不准确”的不满意属于已声明的能力缺口，不能通过反复重生成前端代码解决。要获得真实翻译效果，必须先新增受控模型服务、Secret、配额和网络能力设计，再形成新的可审批范围。
