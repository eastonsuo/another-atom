...
当前实现只有 Blueprint Artifact 和 adapted 请求的最小批准路径，尚未满足本文：

- 没有代码库内真实 `docs/product-spec.md` 与不可变 generation 的绑定；
- supported 方案仍可能跳过首次产品基线确认；
- 没有简介与 Markdown 的一致性 hash；
- 没有编辑后的 `needs_regeneration`、stale 和重新生成流程；
- 没有 ProductSpec 专用的 `workflow` Approval subject；
- 没有把“已批准简介 + 完整 Markdown”作为 Architect Handoff；
