...
- **Agent 交互**
  - 用户通过 Lead 直接获得回答或进入固定团队，并能够检查 Blueprint 和关键阶段产物。
- **产品闭环**
...
- 每条消息只产生 `direct` 或 `team` 两种路由：direct 由 Lead 回答或提出澄清；team 调用 Product Manager、Architect、Engineer、Data Analyst、Reviewer 完整固定团队。
...
- V1 接受任意软件产品需求。Product Manager 必须保留原始项目类型和目标平台，并根据当前是否存在对应源码、构建和运行适配器输出 `support_level`：`supported`、`adapted` 或 `unsupported`。
- `supported` 展示 Blueprint 后继续固定团队；`adapted` 必须列出被替换、忽略或映射的需求，并等待用户确认。
- `unsupported` 不为原请求创建 Build Job。Product Manager 必须保留原始产品目标和项目类型，说明当前缺少的 Runtime Adapter 或工程能力；不得自动生成 Web 替代品。用户可以继续补充需求或要求 PM 重新整理，只有形成当前 Runtime 可执行且经用户确认的方案后才能进入后续构建。
...
| 判定 | 边界规则 | 典型输入 | 系统行为 |
| --- | --- | --- | --- |
| `supported` | 主要目标可以由当前项目类型对应的 Source/Build/Runtime Adapter 完成 | 当前已实现示例：“创建网页扫雷游戏”“制作 Web 看板” | 展示 Blueprint 后自动继续固定团队；网页项目完成后提供 Preview |
...
- 团队是固定顺序角色接力：Product Manager、Architect、Engineer、Data Analyst、Reviewer 使用独立 instruction 和结构化输出；Runtime Validator 是确定性非 Agent 阶段。
- Product Manager 生成 Blueprint；Architect 生成 ArchitectureSpec；Engineer 生成 AppSpec；Data Analyst 只分析结构化数据、内容记录和本地状态，生成 DataProfile；Runtime Validator 生成不可由模型改写的 ValidationReport；Reviewer 基于全部 Contract 与 Evidence 生成 ReviewReport。
- 确定性 ValidationReport 是工程质量门禁；Reviewer 不能把失败改写为通过。Reviewer verdict 不是 `accept` 或存在 blocker 时不创建 ProjectVersion。
- DataProfile 必须提供来源、实体、数据检查、洞察和 warning；ReviewReport 必须提供需求覆盖、工程检查、数据发现、Evidence 引用、问题、warning 和可执行建议。
- Data Analyst 不承担代码审查，Reviewer 不替代确定性 Validator。两个角色任一 Provider 调用失败都必须形成明确失败或降级事实，不能伪造对应 Artifact。
...
```text
 ...
 Message --> Lead --> Product Manager --> Blueprint --> Risk Policy --> Architect --> Engineer --> Data Analyst --> Validator --> Reviewer --> Viewer
              |            |                    |           |          |             |            |
           requirements  edit/approve       ArchitectureSpec    AppSpec      DataProfile   Evidence  ReviewReport
```
...
```text
 ...
 Lead routing --> Direct complete
       |
       `-> Team queued --> Blueprint --> Awaiting risk confirm? --> Building --> Data Analysis --> Validating --> Reviewer --> Complete
                              |                    |              |             |          |
                              v                    v              v             v          `-> Complete degraded
                           Stopped              Failed      Data warning  Validation issue
                                                   |            |
                                             Retry / Remix    Resolve
...
- Lead routing -> Direct complete，或 Team queued -> Blueprint -> Awaiting risk confirm（可选）-> Building -> Data Analysis -> Validating -> Reviewer -> Complete/Complete degraded。
- 任意执行阶段可进入 Stopped。
- Building 失败进入 Failed，提供 Retry 或 Remix；已成功构建后的 Validation issue 才提供 Resolve。
- Complete degraded 只表示 mandatory checks 与 Reviewer acceptance 已通过但存在非阻塞 data/review warning，必须在 UI 明确标注。
...
3. Blueprint 为 supported，基础预算内不重复审批；Architect、Engineer、Data Analyst、Validator、Reviewer 按固定顺序接力，产生 ArchitectureSpec、AppSpec、DataProfile、ValidationReport、ReviewReport。
...
20. 正常 team route 产生可消费 DataProfile 与 ReviewReport；Reviewer rework/needs_input 或 blocker 不得创建 ProjectVersion。
21. Publish 面板持续显示线上/编辑版本；Always Latest 保存动作明确提示会更新线上内容。
22. Provider 请求用量按账户持续记录；超过配置中的历史额度字段也不会阻断 Lead 或构建流程。
...
5. 同意 V1 内部使用 Lead、Product Manager、Architect、Engineer、Data Analyst、Reviewer 等统一角色名称；用户界面主入口显示“和 AI 团队沟通”，Lead 的中文署名为“团队负责人”、英文署名为“AI Lead”。Lead 只做 direct/team 二选一路由，Runtime Validator 是 Data Analyst 与 Reviewer 之间的确定性非 Agent 阶段。
