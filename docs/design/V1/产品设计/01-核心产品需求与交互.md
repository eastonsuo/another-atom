...
V1 交付 Cloud 形态的自包含浏览器应用生成产品：用户通过 Lead 进入固定团队，检查 Blueprint 和阶段产物，在同一 Project 中预览、修改、保存 Git 版本并发布。V1 不实现用户电脑上的本地 Agent Runtime，也不把动态 TaskGraph、开放 Tool 或自主并行写成当前能力。
