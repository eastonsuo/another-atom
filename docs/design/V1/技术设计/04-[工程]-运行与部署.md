...
- **部署组成**
  - V1 部署涉及 Railway 控制面、持久化 Volume、Provider 配置和 Linux Sandbox Host。
- **验收边界**
  - 本文只把能够按步骤验证的能力写成当前可用功能；各部署组成的完成状态以实际环境证据为准，不能由配置说明替代。

本文只描述当前仓库已经实现的 V1 纵切版本。项目、Session、角色产物、配额、事件、版本和发布流程真实运行并持久化。

> 当前运行说明与最终部署设计必须区分。现有纵切已经实现 Lead `direct/team` 路由和风险驱动 Blueprint 处理：`supported` 自动继续，`adapted` 等待确认，`unsupported` 由 PM 生成可确认的替代草案；[V1 系统架构](./03-[工程]-系统架构.md)还定义了正式部署所需的 Project 本地 Git、xterm.js + 受限 Vim 和独立 Linux Sandbox Host。本文只把已经可以按步骤验证的能力写成当前可用功能。
