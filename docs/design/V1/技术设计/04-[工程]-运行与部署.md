...
仓库默认使用 Mock Provider；配置 `OLLAMA_API_KEY` 和 `LLM_PROVIDER=ollama` 后使用 Ollama Cloud。本文只把能够按步骤验证的能力写成当前可用功能；Railway 控制面、持久化 Volume、真实 Provider 和 Linux Sandbox Host 的完成状态以实际环境证据为准，不能由配置说明替代。
