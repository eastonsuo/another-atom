...
开发环境启动时会初始化只读管理员 `admin / admin12345`，密码只以 PBKDF2 哈希存入数据库。V1 公开演示允许保留默认账号，但服务会记录 `default_admin_credentials_active` 告警；正式对外使用建议在 Railway Variables 中覆盖：
...
服务重启时会将这组变量同步到数据库中的管理员账户，并把其他遗留 admin 角色降级为普通用户。普通注册用户不能通过页面将自己升级为管理员。生产环境未设置 Secure Cookie 时会记录 `session_cookie_secure_disabled` 告警。
...
| `ADMIN_USERNAME` / `ADMIN_PASSWORD` | `admin` / `admin12345` | 独立管理员凭据 | 启动时同步唯一管理员；V1 演示允许默认值，但生产会记录告警 |
