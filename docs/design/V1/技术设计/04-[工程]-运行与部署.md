...
OLLAMA_VISION_MODEL=qwen3.5:397b
...
| `OLLAMA_VISION_MODEL` | 空 | `qwen3.5:397b`（2026-07-16 已验证） | 图片参考的视觉解析模型；必须以 Ollama Cloud `/api/tags` 当前返回为准，空值时图片入口不可用 |
| `OLLAMA_VISION_HOST` / `OLLAMA_VISION_API_KEY` | 空 | 通常留空 | 可选的视觉 Provider 覆盖；留空时复用现有 Ollama Host 和 Key |
| `VISION_TIMEOUT_SECONDS` | `120` | `120` | 图片批次视觉解析的独立超时 |
| `ATTACHMENT_STORAGE_ROOT` | `data/attachments` | `/app/data/attachments` | 原始图片的私有存储目录；Railway 必须挂载到持久化 Volume |
...
OLLAMA_VISION_MODEL=<Railway 中实际可调用的视觉模型标识>
VISION_TIMEOUT_SECONDS=120
OLLAMA_TIMEOUT_SECONDS=120
OLLAMA_LEAD_TIMEOUT_SECONDS=60
OLLAMA_FAILOVER_TIMEOUT_SECONDS=30
