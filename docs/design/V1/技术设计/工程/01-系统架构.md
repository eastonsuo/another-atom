...
- Agent 设计：[Another Atom V1 多 Agent 设计](../Agent/01-多Agent设计.md)
...
本节只描述 Agent 与工程 Runtime 的集成摘要。执行范式、角色 Contract、Human-in-the-loop、Context、Tool、Sandbox、验收和有限修复以 [V1 多 Agent 设计](../Agent/01-多Agent设计.md)为事实来源。
...
- Agent 验收权、自动修复条件和失败收敛规则见 [V1 多 Agent 设计：Agent 错误、验收与有限修复](../Agent/01-多Agent设计.md#8-agent-错误验收与有限修复)。
...
Agent Provider、结构化输出、Stage retry、Validation repair 和 Rework 的语义归 [V1 多 Agent 设计](../Agent/01-多Agent设计.md)管理。本章只定义 Build Worker、持久化、事件和发布基础设施如何失败与恢复。
