...
**长期职责：** Engineer 根据项目类型生成对应 SourceBundle，并交给匹配的 Build/Runtime Adapter。**当前实现：** Engineer 只支持 Web `AppSpec`，输出必须能被 Runtime 组合为单个 sandboxed HTML 文档；不得使用远程资源、网络请求、动态 import、`eval`、包依赖、后端调用或 Shell 命令。非 Web 项目在专用 Contract 和 Adapter 落地前不能进入这一阶段。
