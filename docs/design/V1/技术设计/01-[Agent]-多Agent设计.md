...
- **入口路由**
  - Lead 只执行 `direct/team` 二选一路由，不动态创建任务图或调整权限。
- **固定团队**
  - Team 按 Product Manager、Architect、Engineer、Data Analyst、Runtime Validator、Reviewer 的固定顺序运行。
- **Artifact 交接**
  - 每个角色只接收当前阶段所需 Context，并通过经 Schema 校验、持久化的 Artifact 向下一阶段交接。
- **验收与恢复**
  - Runtime Validator 固定工程证据，Reviewer 独立复核；阶段失败采用有限重试和 Artifact 检查点恢复。
- **控制边界**
  - 身份、权限、配额、代码写入和发布由平台 Runtime 控制；动态 TaskGraph、角色自主委派和开放 Tool 属于 V2。
...
项目类型不由 Agent 流程预设。Lead 和 Product Manager 必须保留用户指定的软件类型与目标平台；进入 Architect/Engineer 后，Runtime 应选择匹配的源码和运行 Contract。当前实现只有 Web AppSpec，因此只有 Web 项目能进入完整生成、Preview 和 Public Route 链路；非 Web 请求不得被改写成 Web，而应在匹配 Adapter 尚未实现时形成明确能力缺口。
...
| Product Manager   | 用户要构建什么类型的软件，当前 Adapter 是否支持？ | `Run.prompt`、`Run.mode`                                                | `Blueprint`        | Blueprint Artifact        |
| Architect         | 如何把 Blueprint 落成目标平台对应的架构？      | `Blueprint`                                                            | 当前为 Web `ArchitectureSpec` | ArchitectureSpec Artifact |
| Engineer          | 目标平台需要哪些源码文件和可执行产物？            | 原始 Prompt、`Blueprint`、`ArchitectureSpec`                               | 当前为 Web `AppSpec` | AppSpec Artifact          |
...
**职责：** Product Manager 保留用户目标、项目类型和目标平台，把 Prompt 整理成可检查要求，并根据当前 Adapter 能力提出 `supported / adapted / unsupported` 判断。LLM 只提出 Blueprint；Risk Policy 和 Runtime 决定是否自动继续、请求确认或停止。缺少非 Web Adapter 时必须说明缺口，不能把需求改写成 Web。
...
### 2.4 Architect：把产品方案收敛为目标平台架构

**长期职责：** Architect 根据 Blueprint 的项目类型选择对应架构 Contract，并保持产品目标与目标平台。**当前实现：** 只有 Web `ArchitectureSpec`，字段仍是页面策略、浏览器本地状态和视觉 Token；这只能处理 Web 项目，不能作为所有软件项目的通用架构 Contract。
...
### 2.5 Engineer：生成目标平台源码；当前实现为 Web AppSpec

**长期职责：** Engineer 根据项目类型生成对应 SourceBundle，并交给匹配的 Build/Runtime Adapter。**当前实现：** Engineer 只支持 Web `AppSpec`，输出必须能被 Runtime 组合为单个 sandboxed HTML 文档；允许 Blueprint 已映射需求所需的公网 API 请求，但不得访问 localhost、loopback 或用户设备本地服务，也不得使用远程可执行资源、dynamic import、`eval`、包依赖、后端调用或 Shell 命令。非 Web 项目在专用 Contract 和 Adapter 落地前不能进入这一阶段。
...
`CompletedDegraded` 只表示确定性检查已通过、Reviewer 已接受，但 DataProfile 或 ReviewReport 仍包含 warning。Provider 失败不会伪造降级产物，而是按失败路径保留已经提交的 Artifact。
...
- **[按项目类型选择 Contract]** V1 Agent 必须保留用户指定的软件类型和目标平台。当前 Web Adapter 只能把明确的 Web 项目收敛为自包含浏览器应用；非 Web 项目不得被改写成 Web。未实现的 Source/Build/Runtime Adapter 必须作为能力缺口暴露，不通过 Prompt 暗示任意技术栈、动态依赖、网络、Shell、后端或自动发布已经可用。
