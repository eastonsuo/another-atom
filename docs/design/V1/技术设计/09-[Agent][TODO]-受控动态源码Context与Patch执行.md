...
- **小步修改**
  - Engineer 只输出绑定当前候选 revision、Manifest、ContextReceipt 和逐文件 `before_hash` 的 `SourceFileChangeSet`；modify/add 返回声明文件的完整最终内容，修改或删除现有文件前必须完整读取该 revision 的文件内容。
- **即时验证**
...
- **恢复依据**
  - 系统保存每次模型动作、读取结果、ChangeAttempt、候选 revision、真实 Diff 和验证报告；Worker 重启后从最后一个已完成检查点继续。
...
Engineer（工程师智能体）-> EngineerAction（工程师动作）
      ...
      |
      `-- ProduceChanges（生成文件变更）-> SourceFileChangeSet（attempt 1）
                                          |
                                          v
...
### 1.1 2026-07-16 实现状态校正

静态文件变更纵切已在本地完成：修改生产路径调用 `create_source_file_change_set(...) -> SourceFileChangeSet`，Runtime 校验并物化声明文件，再从真实候选文件重建 AppSpec、SourceBundle 和 SourceDiff。候选的 Build、Unit Test 或 Validator 失败且 Runtime 判定为可修复代码问题时，系统根据当前失败候选生成有界 `SourceRepairContext`，最多接受两次 Repair ChangeSet，每次都重跑完整门禁。旧 Patch Artifact/Event 只读兼容，任何新路径都不回退 raw diff 或完整 AppSpec。
...
- SourceFileChangeSet 即使成功物化，也可能基于错误的局部理解改变行为；
- replacement content 可能超出输出限制或违反候选 Contract；
- ChangeSet 即使成功物化，也可能产生语法、构建、单元测试或验收失败；
- 如果验证错误不能回到 Engineer，系统只能终止，不能利用已经取得的确定性反馈修正候选。

目标实现将静态 `SourceContext` 降级为迁移兼容对象，并以 `BaseSourceManifest + CandidateRevision + RepositoryMap + SourceContextReceipt + ChangeAttempt` 作为长期 Contract。静态 `SourceFileChangeSet` 已替换 raw `SourcePatchSet`，初始与 Repair attempt 链已能持久化；下一步用 `EngineerAction` 接入动态读取，并把当前候选 hash/revision seam 扩展为完整 CandidateRevision 和 ContextReceipt。
...
- 生成并校验绑定当前 candidate revision 的 SourceFileChangeSet；
- 在隔离候选工作区物化声明文件，形成不可混用的 CandidateRevision；
...
小步 ChangeSet 不逐次写入 Project Git，而是在同一隔离候选工作区形成单调递增的候选 revision：

```text
CandidateRevision
  run_id
  revision                     # 0 为基线；成功物化后依次为 1、2、3
  parent_revision?
  base_version_id
  base_git_commit
  source_bundle_ref
...
- ChangeSet 只有通过 Schema、路径、hash 和候选物化检查后才生成下一个 revision；物化失败时 revision 不前进；
- 每个 revision 都有完整 SourceBundle、Manifest 和 RepositoryMap，Repair ChangeSet 必须绑定它实际读取和修改的 revision；
...
- Worker 可以从基线和已接受 ChangeSet 链重建 revision，并用持久化 SourceBundle/hash 校验结果。

`BaseSourceManifest` 始终用于绑定用户批准的原始基线；`CandidateRevision.source_manifest_hash` 用于绑定当前 ChangeSet 输入。两者不能用同一个字段含混表达。
...
2. 若全部受控源码字符数不超过 `MAX_SOURCE_CHARS`，初始工作集包含全部完整文件，Engineer 可以第一轮直接产生 ChangeSet；
...
候选物化或验证失败后，Runtime 不把无界终端日志直接塞回 Prompt，而是生成结构化 RepairContext：
...
- 候选物化失败时没有新 revision，RepairContext 继续绑定原输入 revision，并包含路径、稳定错误和受控摘录；
- Build、Unit Test 或 Validator 失败时已经存在新的候选 revision，RepairContext 绑定该失败 revision；
- Runtime 自动把上一 ChangeSet 实际改变的文件以当前 revision 的完整内容加入修复工作集；内容过大无法加入时明确失败，不把旧内容冒充当前内容；
...
```text
EngineerAction = NeedContext | ProduceChanges | CannotProceed
...
它表示当前 revision 的 Context 不足以安全生成 ChangeSet。`summary` 只解释读取目的，不作为文件权限依据。Runtime 校验请求后从同一 revision 执行允许的读取，并把结果加入下一次 Engineer Context。修复阶段不能借 `NeedContext` 切换回原始基线或读取另一个候选 revision。

### 6.2 ProduceChanges

```text
ProduceChanges
  action: produce_changes
  phase
  change_attempt_index
  source_revision
  input_context_hash
...
它表示 Engineer 已完成当前 attempt 的信息收集并提交候选 ChangeSet。Runtime 仍需校验 Context 覆盖、输入 revision、Manifest hash、路径、Capability Policy 和候选物化结果；动作类型不代表 ChangeSet 已被接受，也不代表构建或测试已经通过。
...
- `action=need_context` 时必须有至少一个合法请求，且不得同时包含 ChangeSet；
- `action=produce_changes` 时必须包含 ChangeSet，且不得同时请求源码；
- `action=cannot_proceed` 时不得包含读取请求或 ChangeSet；
- action 的 `phase / change_attempt_index / source_revision / input_context_hash` 必须与当前 Orchestrator 状态完全一致；
...
- 对应阶段的扩展次数耗尽后，下一次 Engineer 调用必须返回 `ProduceChanges` 或 `CannotProceed`；继续返回 `NeedContext` 时产生 `SOURCE_CONTEXT_ROUND_LIMIT`；
...
- ChangeSet 成功物化后无论验证通过与否都会生成一个新 CandidateRevision；
- 只有 Runtime 判定为可修复的代码失败、仍有 ChangeAttempt、配额和 Context 预算时才进入下一次修复；
- 第三次 ChangeSet 后仍未通过时返回 `SOURCE_CHANGE_ATTEMPT_LIMIT`，不再调用 Engineer；
...
- 读取或 ChangeSet 上限仍不足，说明任务范围、仓库地图、测试反馈或 Adapter 能力不够，继续循环只会增加成本并放大错误；
- 后续是否提高上限必须由真实失败样本和评估证明，不能仅因模型请求继续而自动放宽。

## 9. SourceContextReceipt

每次 `ProduceChanges` 对应一个冻结的 `SourceContextReceipt`（下文简称 ContextReceipt），证明 Engineer 在该 source revision 上实际看到了什么：
...
ContextReceipt 的作用不是声称“相关文件已经全部找到”，而是提供可检查事实：模型在指定 revision 获得了哪些完整文件、哪些片段、做过哪些搜索、哪些请求被拒绝，以及生成本次 ChangeSet 时的最终 Context hash。
...
- SourceFileChangeSet 必须绑定同一 `change_attempt_index / source_revision` 的 `receipt_hash`，不能复用其他 Run、其他 revision 或更早 attempt 的 Receipt。

## 10. SourceFileChangeSet Contract
...
- `modify`：目标文件必须存在于当前 revision 的 `ContextReceipt.fully_read_files`，`before_hash` 与当前文件一致，并提供完整 `replacement_content`；
...
- replacement content 超过配置上限时明确失败，不截断、不回退 raw diff。

### 10.2 小步修改规则

- 每个 ChangeAttempt 只处理当前失败假设直接涉及的文件；Runtime 根据 Policy 校验文件数、单文件大小和物化后的实际 SourceDiff；
- Repair ChangeSet 必须通过结构化字段绑定上一轮 RepairContext 和 failure code，并在 `summary` 中解释修复目标；无新增错误证据时不得重复提交等价 ChangeSet；
- 如果实际修改超过已批准范围或大改阈值，Runtime 暂停并进入 pre-commit Approval，不能因为还有 repair attempt 就自动继续；
- `replacement_content` 是被声明修改文件的完整最终内容，但不代表整库重写；模型不得复制未修改文件；
- 多个 ChangeAttempt 共同形成一个候选 change chain，不分别创建 Git commit；最终 SourceDiff 始终相对原始基线计算。

### 10.3 ChangeSet 不是最终证据

模型提供的文件最终内容只是候选指令。Runtime 不接受模型自报的 changed files、line stats、测试通过或兼容性结论。每个 CandidateRevision 的 SourceDiff、SourceBundle、ExecutionReport 和 ValidationReport 都必须从物化后的真实候选工作区生成。

## 11. Provider 接口

目标接口不再把“读取源码”和“生成文件变更”拆成互不相干的 Provider 方法，而是统一为一个带判别字段的动作：
...
Mock、Ollama/DeepSeek 和未来 Provider Adapter 必须共享同一 Pydantic Contract。初始修改和验证修复调用同一个 `create_engineer_action()`，通过 `phase` 与 RepairContext 区分，不再保留绕开 ChangeSet Contract 的 `repair_app_spec()`。Provider fallback 只能重试相同 Engineer Context，不能在 fallback 时改变文件范围、source revision、attempt 或 Policy。
...
CandidateRevision(0 = base)
  ...
  -> EngineerRunning
       ...
       `-- ProduceChanges
              |
              v
         SourceChangeMaterializing(attempt=N, input_revision=R)
              |
              +-- 物化失败且可修复、N < 3
...
- 候选物化失败不生成新 revision；Build/Test/Validator 只有在物化成功并冻结新 revision 后才执行；
- 每次候选物化成功后都执行完整验证阶梯，不因为它是 Repair ChangeSet 而只运行失败过的单项；
- Runtime 根据错误来源决定 `repairable`，模型不能自行要求进入下一轮；
- 修复始终基于最新成功 apply 的 candidate revision。模型不能选择任意旧 revision，也不能直接操作 Git 回退；
- Stop/Cancel 在任何等待 Provider、读取、候选物化和验证阶段都能终止后续动作，但不删除已持久化证据；
- Runtime 只接受当前 `run_id / phase / change_attempt_index / source_revision / input_context_hash` 对应的动作，迟到结果不能覆盖新状态；
- 达到 ChangeSet、Context、配额或时间上限后停止，不能回退 raw diff 或完整 AppSpec 重生成，也不能创建未经验证的版本。

### 12.1 验证失败分类

Runtime 只把以下确定性代码问题标为可修复：

- replacement content 违反候选源码 Contract，但错误可定位且仍在批准范围内；
- 候选源码语法、类型或固定构建检查失败；
- 项目单元测试失败，且报告能够定位到当前候选源码；
- Validator 给出明确、可操作且仍在已批准范围内的失败项。

以下情况不进入 Repair ChangeSet：
...
- `SourceFileChangeSet`、对应 ContextReceipt 与 ChangeAttempt 的 `change_recorded` 状态在同一事务冻结，ChangeSet 不得指向仍会变化的 Receipt draft；
- 候选物化后 SourceBundle、两个 SourceDiff、CandidateRevision 和 ChangeAttempt 输出字段在同一事务建立引用；
- Build/Test/Validator 报告与 ChangeAttempt 的 `passed / validation_failed` 状态在同一事务提交；
...
2. 读取已完成 Exchange、ChangeAttempt 和 CandidateRevision，按 sequence/attempt 重建当前 revision 与 ContextReceipt draft；
...
5. 已冻结 SourceFileChangeSet 但尚无物化结果时，从 input revision 的持久化 SourceBundle 创建干净临时目录并重新执行确定性文件变更；不在残留 worktree 上猜测；
6. 已存在 CandidateRevision 和 SourceDiff 时不重复物化；已有 ExecutionReport/ValidationReport 时不重复对应验证；
7. 已记录可修复失败但下一 attempt 未开始时，从 RepairContext 继续；
8. 最终通过 revision 已物化版本时复用现有 ProjectVersion/Git 映射，不重复 commit。

任一已持久化 SourceBundle、revision、Receipt、ChangeSet、Diff 或报告 hash 无法重建时进入 `CANDIDATE_RECOVERY_MISMATCH`，不能忽略差异继续。
...
Project bare/local Git
      ...
      `-- base_git_commit
              ...
              `-- candidate worktree / run_id
                      |-- revision 0: base
                      |-- revision 1: initial ChangeSet materialized
                      |-- revision 2: Repair ChangeSet materialized
                      `-- revision 3: final Repair ChangeSet materialized
```
...
1. 校验 SourceFileChangeSet Schema、attempt、输入 revision、CandidateRevision、Manifest 和 Receipt hash；
2. 规范化路径并拒绝绝对路径、`..`、符号链接、submodule、文件模式和不受支持的文件类型；
3. 校验 modify/delete 文件已在输入 revision 被完整读取且 before hash 匹配；
4. 校验 add/delete/modify、replacement content 大小是否在 Approval、Adapter 和 Capability Policy 范围内；
5. 从输入 CandidateRevision 的完整 SourceBundle 建立干净候选目录，按 ChangeSet 确定性写入、添加或删除声明文件；
6. 重新枚举完整受控源码，确认 `changes[]` 外文件相对输入 revision 保持 byte-level 相同；
...
10. 全部门禁通过后进入 VersionMaterialization；可修复失败则生成 RepairContext 并基于当前 revision 开始下一 ChangeAttempt。

候选物化不在共享 Runtime Executor 中直接修改 Project Git。主服务 Repository Service 负责 Git 候选事实；Runtime Executor 只接收候选 SourceBundle，在自己的临时目录执行固定 Adapter。
...
sequence / phase / change_attempt_index / source_revision / round_index / action_type
...
| `SOURCE_CONTEXT_REQUEST_INVALID` | EngineerAction 或请求字段无效，有限 Schema retry | 不读取文件，不创建 ChangeSet |
...
| `CANDIDATE_RECOVERY_MISMATCH` | 恢复重算的 revision、Receipt、ChangeSet、Diff 或报告 hash 不一致 | 停止恢复，当前版本不变 |
| `SOURCE_CHANGE_CONTEXT_VIOLATION` | modify/delete 文件未完整读取或 Receipt 不匹配 | 拒绝 ChangeSet，不进入候选物化 |
| `SOURCE_CHANGE_PATH_FORBIDDEN` | ChangeSet 路径、类型或 protected 状态越权 | 拒绝 ChangeSet |
| `SOURCE_CHANGE_BASE_MISMATCH` | base、input revision、Manifest、Receipt 或 before hash 不匹配 | 拒绝 ChangeSet；不得套用到其他 revision |
| `SOURCE_CHANGE_CONTENT_INVALID` | operation 与 replacement content 不匹配或内容不合法 | 不生成新 revision；有预算时生成 RepairContext |
| `SOURCE_CHANGE_OUTPUT_TOO_LARGE` | 文件数、单文件内容或 ChangeSet 超过上限 | 明确失败，不截断、不回退 raw diff |
| `SOURCE_CHANGE_ATTEMPT_LIMIT` | 第三次 ChangeSet 后仍未通过 | 丢弃候选，不再调用 Engineer |
| `EMPTY_CHANGE` | 候选物化后没有真实源码变化 | 不创建版本 |
| `BASE_VERSION_CONFLICT` | 最终 CAS 时当前版本已变化 | 候选不接入当前版本 |
| `EXECUTION_FAILED` | 构建或单元测试出现可定位代码失败 | 有预算时进入 Repair ChangeSet；否则不创建版本 |
| `EXECUTION_PLATFORM_FAILED` | Executor、存储、租约、超时或平台依赖失败 | 不让 Engineer 猜测修复，按平台恢复/失败处理 |
| `VALIDATION_BLOCKED` | Validator 给出可操作代码失败 | 有预算且未扩大范围时进入 Repair ChangeSet |
| `REPAIR_SCOPE_EXPANDED` | Repair ChangeSet 的累计 Diff 超出已批准范围 | 暂停并请求 Approval，不自动继续 |
...
- CandidateRevision 对相同 parent、ChangeSet 和候选 SourceBundle 生成稳定 hash；
...
- SourceFileChangeSet 必须绑定当前 change attempt、source revision、candidate hash、Manifest 和 Receipt；
- ChangeAttempt 状态、input/output revision 和报告引用组合严格校验；
...
- initial Round 0 直接 ProduceChanges，只调用一次 Provider；
- initial Round 0 NeedContext 后 Runtime 读取，下一轮 ProduceChanges；
- initial 连续两次 NeedContext 后 ProduceChanges，第三次返回 round limit；
- RepairContext 后允许一次 NeedContext 再 ProduceChanges，第二次 NeedContext 返回 repair round limit；
...
### 21.4 ChangeSet 与候选测试

- ChangeSet 修改当前 revision 未完整读取文件时返回 `SOURCE_CHANGE_CONTEXT_VIOLATION`；
- before hash、attempt、source revision、candidate hash、Manifest 或 Receipt 不匹配时拒绝候选物化；
- add/modify/delete、空 ChangeSet、越权路径、超限内容和 symlink 均覆盖；
- 候选物化失败不生成 CandidateRevision；成功后 revision 单调递增；
- 候选物化后 `changes[]` 外文件相对 parent revision byte-level 相同；
- IncrementalSourceDiff 与 parent/child 差异一致，CumulativeSourceDiff 与原始基线/当前候选差异一致；
- 初始 ChangeSet 一次验证通过，只创建一个版本和 commit；
- 初始 ChangeSet 构建失败后 Repair ChangeSet 修复并通过，两次尝试仍只创建一个版本和 commit；
- 候选物化失败后基于同一 revision 修正；测试失败后基于新的失败 revision 修正；
- RepairContext 自动包含上一 ChangeSet 改变文件的当前完整内容和结构化诊断；
- 平台失败、需求歧义、能力缺口和无新证据重复失败不进入 repair；
- 第三次 ChangeSet 仍失败返回 `SOURCE_CHANGE_ATTEMPT_LIMIT`，候选被丢弃且当前版本不变；
- 实际 Diff 扩大批准范围时暂停在 pre-commit Approval；
- 每个成功物化的 revision 都从 Build 起完整复验；
- 最终版本 CAS 冲突不覆盖新版本；
- 成功创建 `ai_edit` 版本但不移动已发布指针。

### 21.5 Railway 部署验收

- 真实 Provider 能完成“一次读取扩展后生成 ChangeSet”和“一次测试失败后生成 Repair ChangeSet”的已有 Project 修改；
- Railway 单副本分别在 ChangeSet、候选物化、CandidateRevision 和验证报告检查点重启后恢复，不重复已持久化动作；
- 持久化 Volume 保留 Project Git、Exchange、Receipt、ChangeAttempt、CandidateRevision 和版本映射；
...
- 首个 ChangeSet 固定 `change_attempt_index=1 / change_kind=initial / input_source_revision=0`；
- 候选物化成功后生成 candidate manifest/hash、Runtime SourceDiff 和 `source_change_attempt_ledger` 记录；完整 CandidateRevision Artifact 仍待迁移；
- 验证失败后已放开一次 initial 加最多两次 repair；修复使用当前失败候选和有界诊断，不再替换 ChangeSet 字段；
...
2. **[已完成｜静态有界修复]** 已增加 `SourceRepairContext`、initial/repair attempt ledger、最多两次 Repair ChangeSet、可修复失败分类和每轮完整 Build/Test/Validation。
3. **[终态 Contract 骨架]** 增加 BaseSourceManifest、CandidateRevision、EngineerAction、SourceReadRequest/Result、SourceContextReceipt 和两类 SourceDiff；将现有 attempt ledger 迁移为完整 ChangeAttempt，保留现有 SourceContext 只读兼容。
4. **[静态 ChangeSet 对齐]** 将当前 `source_context_hash` 迁移为 revision 0 ContextReceipt，并把现有 candidate hash/source revision 映射为完整 CandidateRevision Artifact。
5. **[初始动态 Context]** 增加 RepositoryMap，把 `build_source_context()` 改造成 InitialSourceContext builder：小仓库全量，大仓库不再按路径填充无关文件。
6. **[Provider 动作接口]** 增加 `create_engineer_action()`，先接 Mock，再接 Ollama/DeepSeek；现有 repair Provider 方法迁移到统一 Action Contract。
7. **[动态读取]** Orchestrator 接入 initial 最多两轮 NeedContext、Tool Policy、累计 Receipt、逐次配额和取消恢复；每次 repair 最多补读一轮。
8. **[候选持久化]** 接入 CandidateRevision SourceBundle、增量/累计 Diff 和动态读取/修复检查点恢复；不依赖临时 worktree 常驻。
9. **[验证反馈补齐]** 将当前 Build/Test/Validator 修复扩展到候选物化失败，并补范围扩大、歧义和能力缺口的明确 Approval/terminal 语义。
10. **[风险与版本]** 接每轮 CumulativeSourceDiff 风险检查、pre-commit Approval、最终 CAS 和 VersionMaterialization；整个 Run 只创建一个版本和 commit。
11. **[切换]** 真实 Provider 和 Railway 的动态读取、测试失败修复及重启恢复验收通过后，将终态路径设为默认；历史 Artifact 继续可读。
...
- EngineerAction、动态读取、ContextReceipt、CandidateRevision、ChangeAttempt 和 SourceFileChangeSet Contract 已在 API、Provider、Orchestrator 和持久化层统一实现；
- SourceFileChangeSet 已在隔离候选工作区按 revision 物化，Runtime 能生成增量/累计 SourceDiff；
- 候选物化、Build、Unit Test 或 Validator 的可修复失败能够形成 RepairContext，并在上限内生成 Repair ChangeSet 后完整复验；
- Context、ChangeSet、Build、Unit Test、Validator、版本 CAS 和发布边界的自动化测试通过；
- Worker 在动态读取、ChangeSet、候选物化、CandidateRevision 和验证各检查点重启后能恢复，且不重复已持久化阶段；
...
- 大文件局部读取与受控局部编辑 Contract；
- Explore 子代理把搜索摘要返回主 Engineer；
- 不同项目类型的 Source、Build、Test 和 Runtime Adapter。

这些能力必须由实际仓库规模和失败样本驱动。向量索引只能改善召回，不能替代基线 hash、读取 Receipt、文件变更权限、隔离候选、完整 Diff 和运行验证。
