...
- 候选物化成功后生成 CandidateRevision 1、Incremental/Cumulative SourceDiff 和 ChangeAttempt 记录；
- 迁移完成前可以把部署策略收紧为 `MAX_CHANGE_ATTEMPTS=1`，验证失败明确终止；后续只放开 RepairContext 和 attempt 上限，不再次替换 ChangeSet 字段；
...
5. **[Provider 动作接口]** 增加 `create_engineer_action()`，先接 Mock，再接 Ollama/DeepSeek；删除生产路径对 `revise_app_spec()` 和 `repair_app_spec()` 的依赖。
6. **[动态读取]** Orchestrator 接入 initial 最多两轮 NeedContext、Tool Policy、累计 Receipt、逐次配额和取消恢复。
7. **[候选持久化]** 接入 CandidateRevision SourceBundle、ChangeAttempt、增量/累计 Diff 和各检查点恢复；不依赖临时 worktree 常驻。
8. **[验证反馈]** 对候选物化、Build、Unit Test 和 Validator 失败分类，生成有界 RepairContext；平台错误、歧义和范围扩大不进入 repair。
9. **[有界修正]** 放开最多三次 ChangeAttempt、每次 repair 最多一轮 NeedContext，并在每个成功物化的 revision 后完整复验。
