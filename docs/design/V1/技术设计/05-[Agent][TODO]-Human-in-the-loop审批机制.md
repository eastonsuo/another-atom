...
本文在保留当前纵切兼容性的前提下，定义通用 Risk Policy 和 Approval Contract。核心问题不是增加更多弹窗，而是保证 Agent 不能自行决定风险门禁、批准只对精确对象有效、等待期间不产生副作用，并且并发确认与进程恢复不会重复执行。

## 摘要

- **决策所有者**
  - Agent 只产出结构化事实和风险提示；确定性 Risk Policy 输出自动授权、请求审批或拒绝，Runtime 执行最终门禁。
- **两阶段判断**
...
Human-in-the-loop 由三个相互独立的组件组成：

```text
Agent Artifact / Runtime Evidence / Requested Operation
                         |
                         v
          ...
          |              |              |
          v              v              v
   auto_authorize  require_approval     deny
          |              |              |
          v              v              v
   Runtime 继续       Approval 事实    Needs input / Failed
                         |
                         v
                 用户决定 + Runtime 复核
                         |
...
- **Risk Policy** 判断是否需要确认，不由 Lead、Product Manager 或 Reviewer 最终决定。
- **Approval** 保存用户对精确风险对象的决定，不直接执行 Git、预算或 Deployment 操作。
- **Runtime Guard** 在真实副作用前重新校验 owner、subject hash、基线、预算和状态，并以幂等方式执行。
...
BUDGET_INCREASE
DESTRUCTIVE_DIFF
DIRTY_WORKTREE_DISCARD
VERSION_RESTORE
PROJECT_DELETE
DEPLOYMENT_PUBLISH
DEPLOYMENT_UPDATE
DEPLOYMENT_UNPUBLISH
...
后三类必须输出 `deny` 或已有错误状态，不能请求用户批准后绕过。
...
Approval
  ...
  trigger_message_id?
  approval_type:
    adapted_scope | scope_change | budget_increase
...
`subject_hash` 由服务端对规范化对象计算，客户端不能指定可信 hash。Approval 卡片中的可编辑 Artifact 被修改后，服务端先创建新 Artifact/version、重新执行 Risk Policy，再生成或自动消除 Approval；不能把编辑后的内容塞进旧 Approval。
...
ProductRunning
  -> BlueprintPersisted
  -> RiskCheck
       |-- supported -> BuildQueued
       |-- adapted -> AwaitingApproval
       |                 |-- approved -> revalidate -> BuildQueued
       |                 |-- rejected -> NeedsInput
       |                 |-- cancelled -> Cancelled
       |                 `-- subject changed -> Stale -> RiskCheck
       `-- unsupported -> NeedsInput
```

当前 `/runs/{run_id}/approve` 作为 Blueprint 兼容入口保留，但内部应逐步转为“找到 pending Approval -> 决定 -> 恢复动作”的通用服务。
...
事件 payload 记录 `approval_id / approval_type / subject_type / subject_hash / reason_codes / policy_version`，不记录完整源码和模型私有推理。事件先持久化，再通过 SSE 推送；重连只补取事件，不重新决定 Approval。
...
- supported + 基础预算 -> auto_authorize；
- adapted、范围增删、预算增加、破坏性 Diff、Restore 和 Deployment -> require_approval；
...
- supported Build 无 Approval，adapted Build 等待并在确认后只创建一个 BuildJob；
...
1. **[Contract 统一]** 新增 RiskDecision、通用 Approval 字段和状态；保留现有 Blueprint endpoint 兼容层。
2. **[Policy 收敛]** 把 support level、预算、操作类型和 deny 条件集中到确定性 Risk Policy。
3. **[决定服务]** 实现通用查询/决定 API、CAS、事件和持久化恢复 Job。
4. **[对话修改]** 接入 Pre-execution 与 Pre-commit Gate、stale 处理和 ProjectMessage 结果卡片。
5. **[高影响动作]** 接入 dirty worktree、Restore、删除和 Deployment operation adapter。
6. **[验证]** 完成并发、重启、配额、双用户和 Railway 持久化验收。

不能先让各 API 自己弹窗并保存一个 `approved=true`。没有统一 subject、hash、Risk Policy 和恢复动作时，不同入口会形成互相不兼容的审批语义。
