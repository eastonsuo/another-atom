...
同步首版在视觉解析前同时持久化用户消息和一条 `lead/agent_update` 状态消息。视觉或 Project Lead 失败时，复用该 Lead 消息保存稳定错误码和 Provider 原因；浏览器重试同一份文字与附件时沿用原 `client_message_id`，服务端复用原用户消息、Lead 消息及已经完成的 `ImageContext`。不能把“请求返回 502”解释为“用户消息未保存”，否则恢复草稿后再次提交会制造重复消息。
...
Project 对话发生在 Build Run 完成之后，其视觉解析和 Lead 状态写入 Project 对话消息，不追加到已经结束的 Build Run 日志。同步首版在对话内提供可展开的模型返回或失败详情；迁移到 `ConversationJob` 后再由独立对话任务事件流承载持久化进度，不能借用旧 Run Event 伪装新任务日志。
...
| 配置 | 语义 |
| --- | --- |
| `OLLAMA_VISION_MODEL` | 明确配置支持图片的模型；未配置时纯文本不受影响，带图消息由 Lead 返回稳定失败 |
...
- Project 图片消息使用稳定 `client_message_id` 幂等重试，视觉或 Lead 失败复用原消息并在对话中持久化可展开错误；
- 未配置 `OLLAMA_VISION_MODEL` 时不显示常驻 Composer 警告；纯文本正常处理，带图消息发送后由 Lead 错误消息展示明确原因并保留原消息供重试；
