...
V1 采用 React Studio、FastAPI/Pydantic 控制面、SQLite 持久化、每 Project 一个服务端本地 Git 仓库和单并发持久化 Worker；WebIDE 与构建依赖受限 Linux Sandbox Host。模型只产生结构化输出，平台负责归属校验、状态事务、代码物化、确定性验证和发布。Railway 可承载单副本控制面，但不能替代需要 namespace/cgroup 隔离的执行宿主机。
