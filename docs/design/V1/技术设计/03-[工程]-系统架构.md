...
V1 不按商品类型选择预制渲染器（Renderer）。工程师（Engineer）生成真实源码包（SourceBundle）与应用规格（AppSpec），运行系统（Runtime）根据项目类型选择已登记适配器，负责确定性物化、构建、测试，以及适配器支持时的隔离预览：
...
- 同一个源码包（SourceBundle）必须物化为语义一致的源码文件；构建、单元测试或校验器（Validator）失败时不得创建项目版本（ProjectVersion）。
