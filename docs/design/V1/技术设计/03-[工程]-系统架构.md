...
- **异步执行**
  - 单并发持久化 Worker、Job lease 和 Artifact 检查点承担构建推进、失败恢复与幂等处理。
- **隔离边界**
  - WebIDE 与构建依赖受限 Linux Sandbox Host，模型不直接获得 Shell、文件系统或发布权限。
- **部署边界**
  - Railway 可承载单副本控制面，但不能替代需要 namespace/cgroup 隔离的执行宿主机。
...
| 项目源码与构建 | 服务端本地 Git + Runtime Adapter；当前实现为 WebCode AppSpec + 离线 iframe Sandbox | Project/Git/Version 不绑定项目类型；当前 Web Adapter 生成 HTML/CSS/JavaScript，并用 CSP、禁网校验和无同源 iframe 控制 Preview 风险 | 非 Web Source/Build/Runtime Adapter 尚未实现；V1 不配置 Git remote，也不接受模型生成的 Shell 命令 |
...
用户使用用户名密码登录 React Visual Studio。Lead Agent 对每条消息只做二选一：直接回答/澄清，或启动 Product Manager → Architect → Engineer → Data Analyst → Reviewer 固定团队；Data Analyst 与 Reviewer 之间由确定性 Validator 固定工程事实。构建产物写入 Project 绑定的服务端本地 Git 仓库；用户既可通过结构化编辑，也可通过 xterm.js 中的受限 Vim 修改源码。保存版本时平台统一校验、构建、提交 Git commit，并将 ProjectVersion 映射到 commit SHA。
...
- 构建链按项目类型选择 Source/Build/Runtime Adapter；当前只实现受限 HTML/CSS/JavaScript 代码包、固定 Web 运行边界和固定校验规则。
- Preview 是 Web Adapter 的输出能力，不是所有 Project 的必备字段。没有匹配 Adapter 的项目不得被改写成 Web；Runtime 应在构建前返回明确能力缺口。
- 当前 Adapter 不安装依赖，也不执行模型生成的 Shell 命令。
...
Data Analyst Agent -> DataProfile
    |
    v
确定性校验 -> ValidationReport
    |
    v
Reviewer Agent -> ReviewReport -> Preview 就绪
    |
    v
[6. 修改、版本与发布]
...
V1 使用一个独立 Lead Agent 和五个专业角色。Lead 只生成 `LeadDecision(route=direct|team)`：direct 负责回答或澄清，team 启动固定顺序 Pipeline。Runtime 校验路由、风险与预算；Lead 不能自由挑角色、并行、返工仲裁或发布。
...
Product Manager Agent -> Blueprint
    ...
    -> deterministic Validator
         |-- pass -> Reviewer Agent -> ReviewReport
         |              |-- accept -> Preview Ready
         |              `-- rework / needs_input / blocker -> run.failed
         `-- fail -> run.failed
...
- Data Analyst 只整理应用数据和本地状态；Validator 负责代码与交互级确定性校验；Reviewer 独立检查交付一致性，但不能覆盖 Build/Validation 结果。
- Reviewer 当前可以拒收并阻止版本创建，但 V1 不自动执行返工；后续修复仍需新的受控任务。
...
| Engineer | Engineer Agent 生成页面元数据与自包含 Web 源码 | `AppSpec`（HTML/CSS/JavaScript） |
| Data Analyst | Data Analyst Agent 整理应用数据、内容和本地状态 | `DataProfile` |
| Runtime Validator | Runtime 物化源码并执行确定性边界校验 | `BuildJob`、`ValidationReport` |
| Reviewer | Reviewer Agent 独立检查需求、架构、数据、实现和工程证据是否一致 | `ReviewReport` |
...
- `agent_stage_runs`：Lead、Product Manager、Architect、Engineer、Data Analyst、Reviewer 各阶段的输入产物、输出产物、模型、用量、尝试次数和状态。
- `artifacts`：不可变 Blueprint、ArchitectureSpec、AppSpec、RevisionSpec、DataProfile、ValidationReport 和 ReviewReport。
...
- `project_versions`：AppSpec、DataProfile、ValidationReport、ReviewReport、构建产物引用和对应 `git_commit_sha`。
...
- 每个成功 Agent 阶段以唯一 Artifact 作为恢复检查点；Artifact 与该阶段用量结算同事务提交。恢复时复用已有 ArchitectureSpec、AppSpec、DataProfile、ValidationReport 和 ReviewReport，不重复结算已提交阶段。
...
1. 定义 LeadDecision、Blueprint、ArchitectureSpec、AppSpec、DataProfile、ValidationReport、ReviewReport、Event、Export 和 Error Contract。
...
- Lead 对每条消息只输出 direct/team；direct 只回答或澄清，team 按 Product Manager、Architect、Engineer、Data Analyst、Reviewer 固定顺序执行，Validator 固定在 Data Analyst 与 Reviewer 之间。
...
- 当前 Web AppSpec 可以包含 HTML/CSS/JavaScript，但不能增加依赖、网络访问、构建命令或扩大工作区边界；非 Web Contract 和 Adapter 落地后必须拥有各自校验规则。
...
- **[进程内调度]** Blueprint 和 Build 使用进程内任务与单 Worker 调度，持久化 Job/Lease 负责恢复；V1 不引入独立消息队列或 PostgreSQL `LISTEN/NOTIFY`。
...
- **[独立 Sandbox Host]** 编辑会话使用临时 worktree 和 restricted Vim；V1 不实现多租户 Sandbox 池、容量调度或 MicroVM，rootless、磁盘和逃逸边界必须在目标 Linux Host 实机验收。
