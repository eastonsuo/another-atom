...
- **文档状态：** V1 技术设计基线；Project 路由、静态源码 Context、SourcePatchSet 与隔离 apply 已完成本地实现，受控动态读取、CandidateRevision、Repair Patch、恢复与 Railway 验收尚未完成
...
当前 V1 已能在用户批准后读取基线源码，由 Engineer（工程师智能体）输出静态 Context 下的 SourcePatchSet，并由 Runtime（运行系统）隔离 apply、构建、测试和校验。但大仓库第一次没有选中相关文件时仍不能补读，Patch 或验证失败时也没有基于真实错误继续修正的候选循环。本文定义基于现有代码继续修改的完整增量闭环；角色集合、交付 Contract（数据契约）和最终验收权仍以 [V1 多智能体设计](./01-[Agent]-多Agent设计.md) 为准。
...
- **阶段协作**
  - Runtime 将完整有效 Contract、全量源码 Manifest、低分辨率 RepositoryMap 和初始源码工作集交给 Engineer；Engineer 在有界轮次内读取、生成小步 Patch，并根据 Runtime 验证反馈生成 Repair Patch。
- **Diff 与质量证据**
  - 每次 Patch apply 后，Runtime 都生成增量/累计 SourceDiff，并执行 Build、Unit Test 和 Validator；最多三次 Patch 全部结束后，只把最终通过的 CandidateRevision 写成一个版本和 Git commit。
- **并发与版本写回**
...
当前 `team` 路径绑定 `base_version_id`，顺序生成 ChangeBrief、RequirementDelta、ArchitectureSpec 和完整修订后的 ArchitectureDesign。Engineer 已从完整候选 AppSpec 输出迁移为静态 SourceContext 下的 SourcePatchSet；Runtime 在隔离候选目录 apply，再从真实文件重建兼容 AppSpec、SourceBundle、SourceDiff、ExecutionReport 和 ValidationReport，最后创建 `ai_edit` 版本。部分旧 Artifact 仍为兼容展示保留，但数据分析师（Data Analyst）与质量评审员（Reviewer）不再进入 V1 目标执行链。静态装箱与单次 Patch 是已实现过渡态，目标继续迁移为全量 Manifest、RepositoryMap、受控动态读取、ContextReceipt、CandidateRevision 和有界 Repair Patch。
...
- 修改生产路径已改为 Engineer 输出绑定基线与静态 SourceContext 的 `SourcePatchSet`，Runtime 在隔离候选工作区校验并 apply，再从真实候选文件重建 AppSpec、SourceBundle 和 SourceDiff；`EngineerAction`、受控动态读取、ContextReceipt、CandidateRevision、PatchAttempt 和 Repair Patch 尚未实现；
...
| 工程阶段 | `ModificationContextPackage`：前述 Delta、完整有效 Contract、全量 Manifest、RepositoryMap、初始源码工作集与累计 ContextReceipt | `EngineerAction`，最终为 `SourcePatchSet` | 使用 starter template、越权读取、执行 Shell/Git、直接写 Repository |
...
截至 2026-07-15，该完整同步修订已接入修改流水线。旧版本缺少 `ArchitectureDesign` 时仍走一次迁移生成；已有文档时不得再只复制旧文档并替换视觉 Token。此处采用完整结构化文档输出，是为了维护设计 Artifact 的一致性；它不等于 Engineer 的源码输出策略。Engineer 迁移为 `SourcePatchSet` 的目标及执行边界见下一节和[受控动态源码 Context 与 Patch 执行](./09-[Agent][TODO]-受控动态源码Context与Patch执行.md)。
...
静态第一阶段已经将 Engineer 从完整 CandidateAppSpec 切换为绑定基线与静态 Context 的 `SourcePatchSet schema_version=1.0`。终态 SourcePatchSet 继续绑定当前候选 revision、Patch attempt 和 ContextReceipt：
...
Runtime 只允许 Project 内规范化相对路径，并验证 `base_git_commit / input_source_revision / input_candidate_revision_hash / input_source_manifest_hash / context_receipt_hash / before_hash`。修改或删除现有文件前，目标文件必须以当前 revision 的完整内容进入 ContextReceipt；只读过搜索摘录、局部行范围或旧 revision 不构成修改权限。模型没有 Git、Shell 或写 Repository Tool，只能通过受控 `list/search/read` 请求取得当前候选源码。

Runtime 从当前 input revision 的完整 SourceBundle 创建隔离候选工作区，对每次 Patch 执行：

1. Schema、路径、文件类型和 Capability Policy 预检；
2. `git apply --check` 或等价确定性检查；
3. 在隔离候选工作区 apply；
4. 从 apply 后真实文件生成下一 CandidateRevision、相对 parent 的增量 Diff 和相对原始基线的累计 Diff；
5. 校验 `app-spec.json` 与 HTML/CSS/JavaScript 的 Runtime Contract；
6. 运行系统构建（Runtime Build）、单元测试（Unit Test）和校验器（Validator）的强制检查全部执行；可修复代码失败在剩余预算内生成 RepairContext，重新进入 Engineer；
7. 最多三次 Patch 全部结束后，只有最终候选完整通过才物化一个 ProjectVersion 和 Git commit。

模型返回的 Diff 是候选修改指令，不是最终 Evidence。Runtime 为每个候选 revision 同时保存 IncrementalSourceDiff 和 CumulativeSourceDiff；用户最终看到和版本绑定的是相对原始基线的累计 Diff：
...
模型可以解释 Diff，但不能提供或修改最终 SourceDiff 事实。Patch 无法 apply 或验证失败时，只有 Runtime 判定为可修复且仍有 attempt/Context/配额时才生成 RepairContext；越权、平台错误、歧义、能力缺口和范围扩大不进入自动修正。任何失败都不能回退为整包 AppSpec 重生成覆盖当前代码。
...
V1 始终发送完整 ProductSpec、Blueprint、ArchitectureDesign、ChangeBrief 和验收条件。源码总量未超过 `MAX_SOURCE_CHARS` 时，InitialSourceContext 可以包含全部受控源码；超过预算时只优先包含用户选择、消息准确路径、错误证据和 Adapter 固定入口，不再用路径升序填入无关文件。Engineer 通过结构化 `NeedContext` 在最多两轮内请求额外的文件列表、字面量搜索或源码读取，最终返回 `ProducePatch` 或 `CannotProceed`。
...
完整有效 Contract
  ...
  -> EngineerAction
       ...
       `-- ProducePatch -> SourcePatchSet(attempt 1, revision 0)
...
- Runtime 按 source revision 生成 ContextReceipt，Provider 请求记录 `phase / patch_attempt_index / source_revision / input_context_hash / source_manifest_hash / repository_map_hash / receipt_hash / used_source_chars`；
...
`create_change_brief / create_requirement_delta / revise_architecture_spec / revise_architecture_design / create_source_patch_set` 已接入 Mock 与 Ollama/DeepSeek Provider。当前修改生产路径使用 `create_source_patch_set`，模型只返回静态 Context 下的 Patch 与 AppSpec 元数据 Delta；Runtime 负责本地校验、隔离 apply 和候选重建。`revise_app_spec` 只保留历史兼容，不再是已有 Project 的正式修改入口。动态阶段的目标接口 `create_engineer_action` 尚未实现；它将返回互斥的 `NeedContext | ProducePatch | CannotProceed`，并复用已经完成的 Patch 执行链。
...
2. Repository Service 从 Patch 绑定的 input CandidateRevision 创建隔离候选工作区；
...
未被 Patch 修改的文件必须相对输入 revision 保持 byte-level 相同。模型 Diff 只描述必要变化；Runtime 不允许格式化全部文件制造噪声，也不允许 Patch 修改未在 `patches[]` 声明的文件。
...
Engineer 阶段的 `ContextExchange / ContextReceipt / PatchAttempt / CandidateRevision` 是有顺序的多条记录，不能塞进 `(run_id, artifact_type)` 唯一约束，也不再使用 `repair_app_spec`。Worker 重启后按 sequence、attempt 和 revision 恢复：已经提交的模型动作、读取、apply、Diff 和验证不重复；只有不存在合法检查点的阶段才继续执行。
...
| 动态读取超过字符或轮次上限 | `SOURCE_CONTEXT_LIMIT` / `SOURCE_CONTEXT_ROUND_LIMIT` | 不继续猜测或生成越权 Patch，当前版本不变 |
| Patch 修改未完整读取的文件 | `PATCH_CONTEXT_VIOLATION` | 拒绝 apply，保留 ContextReceipt 和 Patch 证据 |
| Patch 包含越权路径或错误基线 hash | `PATCH_PATH_DENIED` / `PATCH_BASE_MISMATCH` | 拒绝 apply，当前版本不变 |
| Patch 无法应用 | `PATCH_APPLY_FAILED` | 不生成新 revision；有 attempt 时生成 RepairContext |
| Patch 达到三次仍未通过 | `PATCH_ATTEMPT_LIMIT` | 丢弃候选，不创建版本 |
| Provider 输出无效 | 有限 Schema retry，耗尽后失败 | 已提交 Artifact 可复用 |
| Build/Test/Validator 可修复代码失败 | Engineer 在剩余 attempt 内生成 Repair Patch 并完整复验 | 未通过前不创建版本 |
...
- EngineerAction 的 NeedContext、ProducePatch 和 CannotProceed 三种分支互斥；相同读取请求从相同 source revision 得到稳定 result hash；
- ContextReceipt 准确记录 source revision、完整文件、局部范围、搜索、拒绝请求和累计字符量；
- SourcePatchSet 绑定 attempt、input revision、CandidateRevision hash、Manifest、Receipt 和逐文件 before hash；
...
- Engineer 输入包含完整 ProductSpec、有效 Contract、preserve/acceptance 条件、全量 Manifest、RepositoryMap 和实际源码工作集；超限时可在最多两轮内请求额外源码，并最终输出 SourcePatchSet 或明确 CannotProceed；
- modify/delete Patch 只作用于当前 revision ContextReceipt 中完整读取的文件，超出 NeedContext、Patch 或字符上限均明确失败；
- 初始 Patch 验证失败后，结构化 RepairContext 能驱动 Repair Patch；每次成功 apply 后从 Build 起完整复验；
- 最多三次 Patch，无论经历几次候选 revision，成功 Run 只创建一个 ProjectVersion 和 Git commit；
...
已完成 ProjectMessage、ProjectLeadDecision、Project Context 组装与 hash、answer/clarify 无 Run、pending change proposal、独立“修改代码”按钮、批准后建 Run/占写锁、Run trigger/base、ChangeBrief、RequirementDelta、完整 ArchitectureSpec 修订、基于旧版完整 ArchitectureDesign 的同步修订、BaseSourceSnapshot、`MAX_SOURCE_CHARS` 确定性静态源码装箱、SourceContext Artifact、静态 SourcePatchSet、隔离 apply、Runtime 重建 AppSpec/SourceBundle/SourceDiff、Project 单写占用、最终基线检查和 `ai_edit` 版本，以及问答、澄清、批准幂等、旧提案失效、并发、失败和双用户测试。静态 SourceContext 与单次 Patch 是迁移起点，不是目标闭环。
...
3. **[动态 Context 与有界修正]** 按[受控动态源码 Context 与 Patch 执行](./09-[Agent][TODO]-受控动态源码Context与Patch执行.md)把现有静态 Patch 纵切扩展为 BaseSourceManifest、RepositoryMap、EngineerAction、ContextReceipt、CandidateRevision、PatchAttempt、增量/累计 Diff 和 RepairContext；复用已实现的隔离 apply，不回退完整 AppSpec。
