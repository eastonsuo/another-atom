...
- **文档状态：** V1 技术设计基线；Project 路由、静态源码 Context 与批准后建 Run 已实现，受控动态读取、SourcePatchSet 和隔离 apply 尚未实现
...
- **阶段协作**
  - Runtime 将完整有效 Contract、全量源码 Manifest、低分辨率 RepositoryMap 和初始源码工作集交给 Engineer；大仓库允许 Engineer 在有界轮次内结构化请求额外源码，最终输出 SourcePatchSet，由 Runtime 在隔离候选工作区 apply。
- **Diff 与质量证据**
...
- Engineer 仍输出完整候选 AppSpec；EngineerAction、受控动态读取、ContextReceipt、SourcePatchSet、隔离候选工作区 apply 与 apply 后真实 Diff 尚未实现；
...
`BaseSourceSnapshot` 是当前实现 Contract。目标实现将“全量基线证明”和“实际交给模型的源码内容”拆成 `BaseSourceManifest / RepositoryMap / InitialSourceContext / SourceContextReceipt`：Runtime 仍能从基线 commit 读取全部受控源码，但模型只获得初始工作集和后续经过 Policy 校验的读取结果。字段、读取协议和迁移规则以[受控动态源码 Context 与 Patch 执行](./09-[Agent][TODO]-受控动态源码Context与Patch执行.md)为准。
...
Runtime 只允许 Project 内规范化相对路径，并验证 `base_git_commit / source_manifest_hash / context_receipt_hash / before_hash`。修改或删除现有文件前，目标文件必须已经作为完整文件进入 ContextReceipt；只读过搜索摘录或局部行范围不构成修改权限。模型没有 Git、Shell 或写 Repository Tool，只能通过受控 `list/search/read` 请求取得基线源码。
...
完整有效 Contract
  ...
  -> EngineerAction
       |-- NeedContext -> Runtime list/search/read -> 再调用 Engineer（最多两轮）
...
- Runtime 生成 ContextReceipt，Provider 请求记录 `input_context_hash / source_manifest_hash / repository_map_hash / receipt_hash / used_source_chars / input_artifact_refs`；
- modify/delete Patch 只能作用于 Receipt 中已经完整读取且 before hash 匹配的文件；
- 第一次 Patch 因格式或 hunk 定位失败时，Runtime 可以把确定性 apply 错误返回 Engineer 修正一次；不能改为整包重生成，也不能无限重试；
...
`create_change_brief / create_requirement_delta / revise_architecture_spec / revise_app_spec` 已接入 Mock 与 Ollama/DeepSeek Provider；`create_engineer_action` 是替换现有 `revise_app_spec` 修改路径的目标接口，尚未实现。它继续使用 Pydantic 结构化输出、Provider fallback 和阶段用量结算，返回互斥的 `NeedContext | ProducePatch | CannotProceed`。只有 ProducePatch 包含候选 SourcePatchSet，任何分支都不具有写仓库权限。
...
- EngineerAction 的 NeedContext、ProducePatch 和 CannotProceed 三种分支互斥；相同读取请求从相同 base commit 得到稳定 result hash；
- ContextReceipt 准确记录完整文件、局部范围、搜索、拒绝请求和累计字符量；
...
- modify/delete Patch 只作用于 ContextReceipt 中完整读取的文件，第三次 NeedContext、越权路径和超预算读取均明确失败；
...
已完成 ProjectMessage、ProjectLeadDecision、Project Context 组装与 hash、answer/clarify 无 Run、pending change proposal、独立“修改代码”按钮、批准后建 Run/占写锁、Run trigger/base、ChangeBrief、RequirementDelta、完整 ArchitectureSpec 修订、BaseSourceSnapshot、`MAX_SOURCE_CHARS` 确定性静态源码装箱、SourceContext Artifact、完整 ProductSpec/有效 Contract/实际源码进入 Engineer、候选 AppSpec、Runtime SourceDiff、Project 单写占用、最终基线检查、`ai_edit` 版本，以及问答、澄清、批准幂等、旧提案失效、并发、失败和双用户测试。静态 SourceContext 是迁移起点，不再是目标 Context 方案。
...
3. **[动态 Context 与 Patch]** 按[受控动态源码 Context 与 Patch 执行](./09-[Agent][TODO]-受控动态源码Context与Patch执行.md)接入 BaseSourceManifest、RepositoryMap、EngineerAction、Context Exchange、ContextReceipt 和 SourcePatchSet，再实现隔离候选工作区 apply 与 Runtime 真实 Diff。
