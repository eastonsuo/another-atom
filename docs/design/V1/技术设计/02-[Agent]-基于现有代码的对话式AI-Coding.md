...
- **阶段协作**
  - Engineer 接收完整有效 Contract 和基线全量受控源码，输出结构化代码 Diff；Runtime 在隔离候选工作区 apply。
- **Diff 与质量证据**
...
| 工程阶段 | `ModificationContextPackage`：前述 Delta、完整有效 Contract、基线全量受控源码 | `SourcePatchSet` | 使用 starter template、执行 Tool、直接写 Repository |
...
V1 目标是让 Engineer 接收完整 ProductSpec、Blueprint、ArchitectureSpec、AppSpec 和基线 commit 下全部受控源码。超过 Provider Context 上限时返回 `SOURCE_CONTEXT_TOO_LARGE` 并停止，不允许静默裁剪后仍声称完成了全量代码修改。
...
| Engineer | Package 中全部有效 Contract、BaseSourceSnapshot 全量受控源码、当前 AppSpec | `.git`、其他用户文件、宿主路径、密钥 |
...
- Context 仍有长度上限。V1 必须传入全部允许源码；超限时进入 `SOURCE_CONTEXT_TOO_LARGE`，不使用模型自行选择文件；
- 未来扩大仓库后，应先增加路径、符号、依赖关系和确定性分包，再设计多轮 Patch，不优先用向量召回替代完整性证明。
...
V1 简易版不做代码检索和分块规划，优先保证“模型看到完整基线”与“修改可确定性 apply”：
...
- Provider 请求记录 `context_hash / source_manifest_hash / input_artifact_refs`；
...
- 全量 Context 超限直接返回 `SOURCE_CONTEXT_TOO_LARGE`，由用户缩小 Project 或等待更大仓库 Adapter。

未来面向更大仓库时，在保持基线、Context Receipt 和 Runtime apply 不变量的前提下扩展：

- 先构建可验证的文件清单、符号索引、依赖图和测试映射；
- Planner 生成 `SourceChangePlan`，列出必须读取和预计修改的文件；Runtime 校验依赖闭包后分批提供源码；
- 多轮 Patch 每轮绑定同一 base/candidate hash，后续轮次读取前轮候选状态，不依赖隐藏模型记忆；
- 不同技术栈由 Runtime Adapter 定义允许文件、构建命令、测试和 Patch 校验；
- Context Receipt 保存每轮实际提供的文档、文件、hash 和遗漏理由；
- 向量检索只能作为补充召回，不能代替文件/符号/依赖完整性检查。

未来方案改变的是大仓库 Context 组织方式，不改变“模型只给 Patch、Runtime 在隔离环境 apply、真实 Diff 由 Runtime 重算”的执行边界。
...
| 全量源码超过 Context 上限 | `SOURCE_CONTEXT_TOO_LARGE` | 不调用 Engineer，不静默裁剪 |
...
- ProjectContextSnapshot 包含当前产品事实和有界对话；ModificationContextPackage 包含全部有效 Contract 和基线全量受控源码，Context hash 稳定；超限明确失败；
...
- Engineer 输入包含完整 ProductSpec、有效 Contract、基线全量受控源码和 preserve/acceptance 条件，并输出 SourcePatchSet；
...
3. **[修改 Context 与 Patch]** 从完整有效 Contract 与基线 Git 全量源码生成 ModificationContextPackage，接入 `create_source_patch`，并实现隔离候选工作区 apply。
