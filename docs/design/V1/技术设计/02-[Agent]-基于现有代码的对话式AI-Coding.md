...
- **阶段协作**
  - Runtime 先组装完整有效 Contract 和基线全量受控源码，本地计算 Context；未超限时完整发送，超限时裁剪后交给 Engineer。Engineer 输出结构化代码 Diff，Runtime 在隔离候选工作区 apply。
- **Diff 与质量证据**
...
| 工程阶段 | `ModificationContextPackage`：前述 Delta、完整有效 Contract、基线源码 Context 与裁剪回执 | `SourcePatchSet` | 使用 starter template、执行 Tool、直接写 Repository |
| 数据/质量分析 | apply 后候选源码、基线与 Runtime SourceDiff | 增量数据/质量 Artifact | 修改候选代码、覆盖确定性证据 |
...
当前以数据库 `ProjectMessage` 和 API `ProjectMessageView` 落地 `project_id / session_id / user_id / run_id / role / message_type / content / payload / created_at`。下面的 `intent / status / idempotency_key / context_refs` 属于 answer/clarify 与防重复提交的后续扩展，尚未实现。
...
尚未实现。当前 Project 修改入口先创建 Run，再在 Worker 内复用首页 `LeadDecision(direct|team)`；因此产品界面不能宣称已经支持项目内默认问答、正确 Context 或执行前授权。
...
首页 `LeadDecision(route=direct|team)` 不直接复用。项目对话需要区分“回答”和“缺少关键条件的澄清”，并在 `propose_change` 时提供结构化 `ChangeBrief`。`propose_change` 只代表建议修改，不能直接创建 Run；用户批准绑定该 ChangeBrief 与基线的 Approval 后才进入执行。
...
6. Validator 与 Reviewer 通过后才物化 ProjectVersion。
...
当前实现没有组装或传递该 Snapshot；浏览器展示了历史消息，不等于 Provider 收到了历史和 Project 事实。
...
V1 先组装完整 ProductSpec、Blueprint、ArchitectureSpec、AppSpec 和基线 commit 下全部受控源码，再由 Runtime 在本地估算该 Provider 请求的输入 Context，并预留 `SourcePatchSet` 输出预算。未超限时完整发送；超限时执行确定性裁剪后继续调用 Engineer。这是 V1 对 Engineer Context 的唯一容量限制。

裁剪不能改变基线事实：`source_manifest_hash` 和各 Contract hash 始终代表完整基线，另以 `included_documents / omitted_documents / included_files / omitted_files / included_hashes / estimated_input_tokens / context_budget / trimming_reason` 形成 `ContextReceipt`，明确记录模型本轮实际看到的文档和源码。文档与源码按完整单元裁剪，不把半份文档或半个文件伪装成完整输入；系统指令、ChangeBrief、验收条件和基线身份属于不可裁剪区，显式选中文档/文件优先，其他有效 Contract 与源码按服务端固定顺序装入预算。这里不引入 Planner、向量检索、多轮分包或额外的大仓库限制。
...
| Engineer | Package 中全部有效 Contract、当前 AppSpec、BaseSourceSnapshot 源码；仅在本地计算超限时按 ContextReceipt 裁剪 | `.git`、其他用户文件、宿主路径、密钥 |
...
- Runtime 必须先计算完整 Package 的 Context；只有超过当前 Provider 输入预算时才调用确定性裁剪器，模型不能自行选择或声称读取了未包含文件；
- Provider 调用、Patch 和后续 Review 必须引用同一 `ContextReceipt`，使裁剪范围可追溯。
...
### 5.6 V1 Context 预算与裁剪

V1 不做代码检索、Planner 或分块规划，只增加本地 Context 计算与超限裁剪：

```text
完整有效 Contract
  + base commit 全量受控源码
  + ChangeBrief / RequirementDelta
  -> Runtime 本地计算 Context
       |-- 未超限：全量发送
       `-- 超限：确定性裁剪 + ContextReceipt
  -> 单次 Engineer Provider 调用
  -> SourcePatchSet
  -> 隔离候选工作区 apply --check / apply
  -> Runtime SourceDiff / Validator / Reviewer
```

- 每个源码文件使用明确路径和边界标记，文件内容作为不可信数据而非 Prompt 指令；
- Provider 请求记录 `context_hash / source_manifest_hash / ContextReceipt / input_artifact_refs`；
...
- 本地估算器使用 Provider/模型对应的 Context window，并预留固定输出预算；估算器版本写入 ContextReceipt；
- 裁剪只解决输入容量，不改变“模型只给 Patch、Runtime 在隔离环境 apply、真实 Diff 由 Runtime 重算”的执行边界。
...
当前 Lead 直接输出 ChangeBrief；Project 消息的 answer/clarify 路由仍待补充，因此不能把现有 `POST /projects/{id}/messages` 描述为完整的通用对话接口。
...
| 完整 Package 超过 Context 上限 | Runtime 本地确定性裁剪并生成 `ContextReceipt` | Engineer 只基于实际发送范围生成 Patch；当前版本不受影响 |
...
- ProjectContextSnapshot 包含当前产品事实和有界对话；ModificationContextPackage 先包含全部有效 Contract 和基线全量受控源码，Context hash 稳定；本地计算未超限时不裁剪，超限时生成稳定的 ContextReceipt；
...
- Engineer 输入包含完整 ProductSpec、有效 Contract、preserve/acceptance 条件，以及未超限的全量源码或超限后的源码 Context；输入与 ContextReceipt 一致，并输出 SourcePatchSet；
...
1. **[对话路由与 Context]** ConversationJob、ProjectLeadDecision、ProjectContextSnapshot、answer/clarify 和消息 idempotency key。
2. **[修改授权]** ChangeBrief 卡片、`project_change` Approval、“修改代码”按钮，以及批准后建 Run/占写锁。
3. **[修改 Context 与 Patch]** 从完整有效 Contract 与基线 Git 全量源码生成 ModificationContextPackage，实现本地 Context 计算、超限裁剪与 ContextReceipt，接入 `create_source_patch`，并实现隔离候选工作区 apply。
