...
- **文档状态：** V1 技术设计基线，核心纵切已实现，剩余差距见第 2 节
...
当前已新增 `GET/POST /api/projects/{project_id}/messages`。修改请求创建 `trigger=ai_edit` 的 Run，自动绑定 `base_version_id`；Worker 从该版本的 Git commit 读取受控源码，顺序生成 ChangeBrief、RequirementDelta、完整 ArchitectureSpec、候选 AppSpec、SourceDiff、DataProfile、ValidationReport 和 ReviewReport，最后创建 `ai_edit` 版本。

已实现的正确性边界包括 Project 单写占用、最终基线检查、阶段 Artifact 复用、阶段配额、失败释放、用户归属校验和发布指针不变。

仍有四项差距：

- Project 消息尚未实现 answer/clarify；当前入口只处理明确修改；
- Risk Policy 尚未根据范围、破坏性 Diff 和额外预算生成 Approval；
...
尚未实现。当前 Project 修改入口把用户提交视为明确 `modify`，Lead 直接生成 ChangeBrief；因此产品界面不能宣称已经支持项目内问答和澄清路由。
...
当前实现只处理已经归入 Another Atom Project Repository 的代码。API 不要求客户端提交 commit 或文件内容；Runtime 根据 `Project.latest_version_id` 自动取得 Git commit，并从该 commit 读取 `app-spec.json`、`index.html`、`styles.css` 和 `app.js`，生成 BaseSourceSnapshot。
...
上述方法已经同时接入 Mock 与 Ollama/DeepSeek Provider，继续使用 Pydantic 结构化输出、Provider fallback 和阶段用量结算。Data Analyst 与 Reviewer 当前复用首次 Build 的 `analyze_data` 和 `review` Contract。
...
已完成 ProjectMessage、修改 API/历史 UI、Run trigger/base、ChangeBrief、RequirementDelta、完整 ArchitectureSpec 修订、BaseSourceSnapshot、候选 AppSpec、Runtime SourceDiff、Project 单写占用、最终基线检查、`ai_edit` 版本，以及主路径、并发、失败和双用户测试。

后续按以下顺序完成：

1. **[对话路由]** ProjectLeadDecision、answer/clarify 和消息 idempotency key。
2. **[风险确认]** impact/risk Contract、范围与破坏性 Diff 判断、Approval UI。
3. **[结果证据]** 独立 Diff/验收结果卡片和候选 hash。
4. **[提交恢复]** VersionMaterialization，收窄 Git commit 成功而数据库事务未完成的窗口。
5. **[接入与部署]** GitHub/本地目录首次导入，以及 Railway 真实 Provider、进程重启和 Volume 验收。
