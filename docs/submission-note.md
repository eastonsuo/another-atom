...
### 3.2 可检查阶段，而不是多 Agent 表演

Planner、Designer、Engineer、QA 只是单 Agent 与确定性工具的阶段语义，每个阶段必须绑定实际产物：
...
前端不声称多个模型并行工作。评审者可以直接检查每个阶段是否改变了后续状态，避免用角色头像和进度动画替代真实执行。
...
| 创新性 | Blueprint/AppSpec/Renderer 受控生成链路、阶段产物和版本化发布 |
