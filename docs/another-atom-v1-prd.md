...
推荐实现 **V1.0 可部署纵切版**：做一个可公开访问、可完成核心任务、状态真实保存的 AI 应用生成 demo；产品交互受 Atoms 启发，但使用 Another Atom 自己的品牌、角色和信息结构。Blueprint 与 AppSpec 由真实 LLM 生成，项目通过受控工具和 React 模板落地；第三方平台能力仍采用受控模拟。

它不是 Atoms 官网复刻，也不是静态高保真原型。用户必须能完整走通：

`从终端或首页输入需求 -> 选择模式 -> 确认 Blueprint -> 查看构建执行 -> 预览应用 -> 修改内容 -> 处理错误 -> 保存版本 -> 发布 -> 打开公开链接`

```text
 +---------+    +---------+    +-----------+    +---------+
 | CLI/Home|--->| Prompt  |--->| Blueprint |--->| Build   |
 +---------+    +---------+    +-----------+    +----+----+
                                                      |
                                                      v
 +------------+   +---------+    +----------+    +----+----+
...
| V1.5 扩展能力版 | V1.0 加入独立执行沙箱或一个真实外部集成 | 能验证一个关键平台依赖 | 成本、稳定性和权限管理复杂度上升 | V1.0 通过后再选一个方向 |
...
1. 用户能否从 Terminal CLI 或首页 Prompt Composer 顺利启动一个项目。
...
### 6.1 Terminal CLI、应用外壳与首页

- Terminal CLI 是主要控制入口，支持创建项目、输入需求、确认 Blueprint、查看构建状态和恢复 Session。
- CLI 启动本地 Agent Runtime，并可打开 React Visual Studio 查看可视化结果。
- CLI 与 Visual Studio 使用同一 Project、Session、Run 和 Event 模型，不能形成两套独立状态。
...
```text
 ...
 Queued --> Blueprint --> Awaiting confirm --> Building --> Validating --> Complete
                              |                    |            |
                              v                    +----+-------+
                           Stopped                      v
                                                   Failed
...
2. Terminal CLI 与 Home Prompt Composer 都能启动项目；浏览器入口可完成输入、附件、模式选择、提交和错误反馈，不存在静态假控件。
...
1. 同意选择 V1.0 可部署纵切版。
2. 同意 Home、Prompt Composer 和 Blueprint 都属于 P0，首页不是营销页。
3. 同意 P0 只执行 Engineer/Team，并使用 Planner、Designer、Engineer、QA 等通用角色。
4. 同意使用 Mono Market 作为唯一 Golden Path，并采用有限模板驱动生成。
5. 同意公开 URL、版本策略和跨浏览器访问属于 P0。
6. 同意 Cloud/Integrations/Growth 只做能力中心，除基础 SEO 结果外不接真实服务。

上述决策已经进入技术设计，架构、数据模型、发布方案、目录结构、测试和部署流程见 [Another Atom V1 架构设计](./architecture-design.md)。
