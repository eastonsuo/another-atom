...
同日补充决定：V1 只用固定 `MAX_SOURCE_CHARS` 解决代码库过大问题。完整有效 Contract 不裁剪；源码未超预算时全量发送，超出时按“选中文件、消息中的准确路径、其余路径升序”装入完整文件，并记录包含/省略清单。不计算 tokenizer，不引入检索、依赖分析或分包。本决定替代本文发现 5 中“超限直接失败”的建议。
