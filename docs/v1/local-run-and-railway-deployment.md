...
> 当前运行说明与最终部署设计必须区分。现有纵切已经实现 Lead `direct/team` 路由和风险驱动 Blueprint 处理：`supported` 自动继续，`adapted` 等待确认，`unsupported` 由 PM 生成可确认的替代草案；[V1 架构设计](./architecture-design.md)还定义了正式部署所需的 Project 本地 Git、xterm.js + 受限 Vim 和独立 Linux Sandbox Host。本文只把已经可以按步骤验证的能力写成当前可用功能。
