...
V1.0 可以理解任意产品目标，并把它实现为自包含的浏览器应用：例如游戏、计算器、看板或商品目录。这里的“任意”描述产品目标，而不是任意技术能力；真实认证、支付、持久化数据库写入、外部服务、原生运行时、动态依赖和 Shell 执行仍属于能力边界，必须被标记为 `adapted` 或 `unsupported`。
...
- V1 接受任意产品需求，但执行目标是自包含 Web 应用。Product Manager 必须保留原始产品类型，并输出 `support_level`：`supported`、`adapted` 或 `unsupported`。
- `supported` 展示 Blueprint 后继续固定团队；`adapted` 必须列出被替换、忽略或映射的需求，并等待用户确认。
- `unsupported` 不为原请求创建 Build Job。Product Manager 必须保留原始产品目标，说明无法由 Web Runtime 表达的能力，并生成围绕同一目标的可检查需求草案。用户可以确认草案直接进入 Architect，也可以显式要求 PM 重新生成；重新生成本身不创建 Build Job。
...
| 判定 | 边界规则 | 典型输入 | 系统行为 |
| --- | --- | --- | --- |
| `supported` | 主要目标可以由自包含 HTML/CSS/JavaScript 和浏览器本地状态完成 | “创建扫雷游戏”“制作计算器”“创建商品目录” | 展示 Blueprint 后自动继续固定团队 |
| `adapted` | 产品目标可保留，但真实认证、支付、数据库写入或外部服务需要改成本地演示状态 | “创建 CRM，并需要真实企业登录和云端客户数据” | 保留 CRM 产品目标；明确省略的服务端能力并等待确认 |
| `unsupported` | 主要目标依赖无法由 Web Runtime 表达的原生系统或硬件能力，替换后会改变产品本质 | “实现只能依赖原生驱动的系统工具” | 停止原请求、说明能力缺口；PM 只能提出保持原目标的 Web 实现草案，不能改成其他产品 |
...
- Product Manager 生成 Blueprint；Risk Policy 检查必要确认；Architect 生成 ArchitectureSpec，Engineer 生成包含 HTML/CSS/JavaScript 的 AppSpec，Runtime 将源码写入 Project Git 并在离线 iframe Sandbox 中预览，Data Analyst 基于 ValidationReport 生成 DataReview。
...
- 页面与路由由 Blueprint/AppSpec 描述；游戏、工具、看板和目录使用同一 Web 源码交付链。
...
真实 LLM 与受限 Web Runtime 结合不代表任意技术栈或任意外部系统集成，但能同时验证需求理解和稳定落地。界面和文档必须明确这一边界：V1 可以生成产品目标各异的 HTML/CSS/JavaScript 应用，不能包装成可自由调用后端、安装依赖或执行 Shell 的通用编程环境。
