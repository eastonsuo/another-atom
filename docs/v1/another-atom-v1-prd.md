...
- Team Mode 是固定顺序的角色接力：Team Leader、Product Manager、Architect、Engineer、Data Analyst 使用独立 instruction 和结构化输出，按顺序消费上一阶段的明确产物。
- Product Manager 根据 Prompt 和附件生成 Blueprint；用户确认后，Architect 生成 ArchitectureSpec，Engineer 生成 AppSpec，固定 Renderer 完成构建，Data Analyst 基于 ValidationReport 生成 DataReview。
...
```text
 ...
 Prompt --> Product Manager --> Blueprint --> Confirm --> Architect --> Engineer --> Build --> Data Analyst --> Viewer
              |            |                    |           |          |       |
...
```text
 ...
 Queued --> Blueprint --> Awaiting confirm --> Building --> Validating --> Data Analyst Review --> Complete
                              |                    |            |             |
...
- Queued -> Blueprint -> Awaiting confirm -> Building -> Validating -> Data Analyst Review -> Complete/Complete degraded。
- 任意执行阶段可进入 Stopped。
...
3. Team Leader、Product Manager、Architect、Engineer、Data Analyst 按固定顺序接力，分别产生 Blueprint、ArchitectureSpec、AppSpec、ValidationReport/DataReview；时间线标注“分阶段接力”。
...
17. 正常 Team Mode 产生可消费 DataReview；deterministic-only 降级明确标注且不计入 Golden Path 成功。
...
5. 同意 P0 只执行 Engineer/Team，并使用 Team Leader、Product Manager、Architect、Engineer、Data Analyst 等统一角色名称。
