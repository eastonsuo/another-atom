...
| --- | --- | --- | --- | --- |
| Team Leader | Run 状态、配额、阶段结果、错误证据 | `StageDecision` 事件 | 固定顺序推进、有限重试、失败收敛 | 不代替专业角色生成 Contract，不代替用户审批或发布 |
| Product Manager | Prompt、附件元数据、平台能力范围 | `Blueprint` | 需求字段、`support_level`、适配说明 | 不确认 Blueprint，不创建 Build Job |
| Architect | 已确认 Blueprint、Renderer Capability Contract | `ArchitectureSpec` | 页面策略、数据实体、布局、视觉和交互约束 | 不增加 Blueprint 未批准的页面或模块，不选择任意技术栈 |
...
```text
Team Leader     -> fixed StageDecision / progress events
...
Engineer Mode 省略 Product Manager、Architect 和 Data Analyst 的独立展示与模型调用，但不能跳过 Blueprint 审批、Pydantic 校验、确定性 Build/Validation 或用户发布门。

## 3. Orchestrator 与执行状态

V1 的 Team Leader 由 Orchestrator 状态机实现，不是自主 Leader Agent。它只依据持久化状态和确定性规则推进：

```text
Created
  -> ProductRunning / EngineerPlanning
  -> AwaitingBlueprintApproval
  -> ArchitectRunning              (Team Mode)
  -> EngineerRunning
  -> BuildQueued
  -> Building
  -> Validating
       |-- pass -> DataReviewing     (Team Mode)
...
| Architect | 已确认 Blueprint、视觉 token 范围 | 原始完整会话、Engineer/Data Analyst 内容 |
| Engineer | 已确认 Blueprint、ArchitectureSpec、Renderer Capability Contract | 任意宿主文件、密钥、未批准需求 |
...
**默认没有可执行 Tool。** 四个专业角色通过显式输入获得所需 Artifact，并只返回经 Pydantic 校验的结构化输出。Provider Adapter 不开放 Tool Calling；V1 的 Run、事件、配额和 Trace 由自有 Runtime 管理。
...
- Team Mode 严格按 Product Manager -> Architect -> Engineer -> Data Analyst 推进，同一时刻只有一个主阶段。
...
- 正常 Team Mode 的 DataReview 包含摘要、warning、Evidence 和可执行建议；`CompletedDegraded` 必须明确标注且不能用于 Golden Path 验收。
