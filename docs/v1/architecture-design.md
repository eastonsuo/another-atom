...
`unsupported` 的 `rewrite_suggestion` 必须是 Product Manager 根据原主题生成的完整可构建需求，而不是通用 Golden Path 提示。界面必须说明它改变了产品类型。原 Run 保持终止且不创建 Build Job；用户确认或编辑草案后创建新 Run，复用已确认 Blueprint 并直接进入 Architect，既避免复用已终止状态，也避免第二次 PM 改写用户确认的内容。
