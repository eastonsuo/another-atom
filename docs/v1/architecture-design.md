...
| Agent 编排 | OpenAI Agents SDK + Pydantic Structured Output | 支持真实 LLM 调用、角色 instruction、Usage/Trace 和结构化校验 | V1 固定顺序执行，不做自主委派或并行协商 |
...
1. **Contract 优先**：Blueprint、VisualSpec、AppSpec、Event、Error 和 Export 以 Pydantic 模型为事实来源。
...
External: LLM Orchestrator -- HTTPS --> OpenAI API
```
...
Product Manager Agent -> Blueprint -> user approval
    ...
    -> deterministic Validator
         |-- pass ----------------------------> QA Agent -> QAReview -> Preview Ready
...
- QAReview 可以解释问题和建议修复，但不能覆盖确定性 Validator 的 Build/Validation 结果。
...
前端可以展示 Product Manager、Designer、Engineer、QA，但每个阶段必须绑定可检查产物：
...
- `agent_stage_runs`：Product Manager、Designer、Engineer、QA 各阶段的输入产物、输出产物、模型、用量、尝试次数和状态。
- `artifacts`：不可变 Blueprint、VisualSpec、AppSpec、RevisionSpec、ValidationReport 和 QAReview。
...
- Worker 只能领取 queued 或 lease 已过期的 Job；`idempotency_key` 防止同一请求创建重复执行。
...
4. 实现 Product Manager、Designer、Engineer、QA 固定顺序 Orchestrator、阶段持久化和配额结算。
...
- Team Mode 按 Product Manager、Designer、Engineer、QA 固定顺序执行，每个阶段都有独立 AgentStageRun 和可检查产物。
