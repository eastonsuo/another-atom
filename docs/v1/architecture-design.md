...
V1 采用 Cloud 产品形态，但把控制面与执行面分开：React 提供可视化工作台，FastAPI 承担 API、Agent 编排和领域服务，SQLite 保存业务事实，Linux Sandbox Host 承担本地 Git、Vim PTY 和受控构建。Railway 只运行单副本 Control Plane，并把 SQLite 与 Project Git 放在持久化 Volume；Control Plane 也可与 Sandbox Host 一起部署到同一台 Linux VM。
...
| 项目源码与构建 | 服务端本地 Git + WebCode AppSpec + 离线 iframe Sandbox | Engineer 生成自包含 HTML/CSS/JavaScript；Git commit 将 ProjectVersion 与源码对应；CSP、禁网校验和无同源 iframe 控制风险 | V1 不配置 Git remote，不安装动态依赖，不接受模型生成的 Shell 命令 |
| 代码终端 | xterm.js + WebSocket + 隔离 PTY + 固定 Vim | 给用户真实键盘编辑体验，同时把修改限制在当前 Project 仓库 | 不启动登录 Shell；禁用 shell escape、插件、网络和仓库外路径 |
| 异步执行 | Sandbox Host 上的单并发持久化 Build Worker | V1 无需额外消息队列即可脱离 HTTP 生命周期，并通过数据库 Job/lease 恢复 | 提高并发前必须在目标 Host 规格压测 |
| 部署 | 单副本 Control Plane + SQLite Volume + Linux Sandbox Host | Control Plane 处理 Web/API/Agent；持久化 Volume 保存状态和 Git；执行宿主机提供 rootless Editor/Build Sandbox | Railway 可承载 Control Plane，但真实 WebIDE 必须有支持 namespace/cgroup 的 Linux Host |
...
└──────────────────────┬───────────────────────┬───────────────────┘
                       │                       │
                       ▼                       ▼
           SQLite + Volume              Linux Sandbox Host
...
- 受限的 HTML/CSS/JavaScript 代码包、固定运行边界和固定校验规则；不安装依赖，也不执行模型生成的 Shell 命令。
...
Worker 获取 Job lease
    |
    v
WebCode Packager -> Project Git：index.html / styles.css / app.js
    |
...
`unsupported` 的 `rewrite_suggestion` 必须保持原始产品目标，只能调整无法由 Web Runtime 提供的技术能力，不能改成商品目录或其他产品。原 Run 保持终止且不创建 Build Job；用户确认或编辑草案后创建新 Run并直接进入 Architect。
...
| Engineer | Engineer Agent 生成页面元数据与自包含 Web 源码，Runtime/Validator 执行物化和边界校验 | `AppSpec`（HTML/CSS/JavaScript）、`BuildJob`、`ValidationReport` |
...
### 5.5 WebCode Packager 与 Preview Sandbox

V1 不按商品类型选择预制 Renderer。Engineer 生成真实 Web 源码，Runtime 负责确定性物化和隔离预览：

- 只接受通过 schema 校验的 AppSpec。
- 只向当前 Project Git 写入 `index.html`、`styles.css`、`app.js`、`app-spec.json` 和版本标记。
- HTML 是语义化 body fragment；交互代码必须位于 JavaScript 字段，不能在 HTML 中夹带 script/iframe/object/embed。
- 禁止远程 URL、fetch、WebSocket、动态 import、eval、运行时依赖安装和模型提供的命令字符串。
- Preview 使用 `sandbox="allow-scripts"` 且不授予 `allow-same-origin`，CSP 将 `connect-src`、外部资源和表单提交设为 none。
- 同一个 AppSpec 必须物化为语义一致的源码文件；Validator 失败时不得创建 ProjectVersion。
...
- 使用数据库状态 CAS 与 lease 防止同一 Job 重复执行。
- 服务重启后重新领取 queued 或 lease 过期的 Job。
- 使用异步子进程执行固定构建命令。
- 每个 Job 设置可配置超时、输出上限和工作区磁盘上限。
- 禁止启用多个 Uvicorn Worker 后各自无协调地消费任务。

当前可运行纵切采用单实例形态：一个 FastAPI 进程、一个进程内 Build Worker、一个 SQLite 事实源。Railway 将数据库和 Project Git 放在 `/app/data` 持久化 Volume。`POST /api/runs` 只提交 Run 并返回 `product_running`；Blueprint 由响应后的 `BackgroundTasks` 使用新数据库 Session 生成，进程启动时重新处理遗留的 `product_running` Run。Build 仍由持久化 `build_jobs` 和 lease 驱动，不依赖 HTTP 请求存活。风险驱动 Approval 已用于 Blueprint 路径：`supported` 自动继续，`adapted` 进入确认，`unsupported` 停止原 Run 并展示 PM 生成的可构建草案；CAS 和唯一约束保证确认不会重复创建 Build Job。
...
Control Plane 与 Sandbox Host 分离后，API 不与 Vim/Build 竞争同一进程资源。V1 仍保持一个 Worker；若未来拆成多 Worker，需先迁移 PostgreSQL 或引入明确的 Job 协议：

```text
Control Plane ------ durable Job protocol ------ Sandbox Host Worker
        |                                     |
        |                                     v
        `---------- Artifact metadata <--- local Git / build / dist
```

源码事实保存在 Sandbox Host 的 Project Git，发布 Artifact 使用 storage key/hash 引用，不能假设 Control Plane 可以直接读取执行宿主机绝对路径。该拆分不属于当前 Railway 单副本 SQLite 的并发模型。
...
- 每个成功 Agent 阶段以唯一 Artifact 作为恢复检查点；Artifact 与该阶段用量结算同事务提交。恢复时复用已有 ArchitectureSpec、AppSpec、ValidationReport 和 DataReview，不重复结算已提交阶段。
...
| Worker 中断、lease 过期 | SQLite Job/lease 状态 | Build Worker 恢复逻辑 |
...
3. 复制 Renderer 与受限 Web Runtime 校验器。
4. 不在运行时执行依赖安装或模型生成的 Shell 命令。
...
1. 在持久化 Volume 上初始化或检查 SQLite schema。
2. 启动单实例 FastAPI 和持久化 Build Worker。
3. Worker 从 SQLite 领取 Build Job。
...
`github.com/eastonsuo/another-atom` 只负责平台自身源码、文档和部署触发。用户 Project 的源码保存在 Sandbox Host 的本地 Git 仓库，不配置 GitHub remote，也不使用平台仓库承载用户代码。GitHub Pages 不能运行 FastAPI、Agent、SQLite 持久化服务、Terminal Gateway 或 Sandbox Manager。
...
│   └── storage/          SQLite repositories and volume paths
...
6. 完成 WebCode AppSpec、源码物化、离线 Preview Sandbox、`build_jobs`、Worker 和持久化事件。
...
- 任意产品需求都必须保留原始产品目标；浏览器内可完成的需求进入 supported，需要服务端/外部/原生能力时进入 adapted 或 unsupported，unsupported 不创建 Build Job。
- 普通 supported 且用户明确要求构建时不重复审批；adapted、额外预算、范围变化、破坏性仓库操作和线上变更必须触发风险确认。
- AppSpec 可以包含 HTML/CSS/JavaScript，但不能增加依赖、网络访问、构建命令或扩大工作区边界。
