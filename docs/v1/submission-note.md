...
### Priority 1：V2 自主多 Agent

- Leader 动态拆解、委派和仲裁任务。
- Agent 独立上下文和工具权限。
- 选择性并行、结构化返工、冲突处理和结果合并。
- Agent 级预算、超时和终止条件。

原因：V2 是已规划的下一实施版本。V1 固定角色接力先验证阶段 Contract；Contract 和可观测性稳定后，自主编排才有可检查的输入、输出和成本边界。

### 未归属版本：CC 式 Terminal 与本地 Agent Runtime
...
原因：这是 Another Atom 从在线生成工具走向本地开发工具的独立扩展方向，但当前不占用 V1 或 V2 的版本定义。
