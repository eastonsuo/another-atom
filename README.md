...
- **Provider 兜底：** Lead 默认通过 Ollama Cloud 调用 DeepSeek V4，并关闭无必要的 thinking。配置 `DEEPSEEK_API_KEY` 后，Ollama 单次请求超过 30 秒才切换到 DeepSeek 官方 API；界面显示“服务商切换中”，Run 阶段保存 `provider.fallback` 事件，两次请求用量一起结算。
