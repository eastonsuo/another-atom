...
- **[固定交接]** 第一版使用产品经理、架构师、工程师三角色顺序流水线；多智能体的价值由可检查产物和明确交接体现，不以角色数量衡量。
...
当首次构建意图缺少会影响产品形态的关键信息时，Lead 会先提供结构化选项；用户完成选择后，原始需求和补充结果一起进入 Product Manager 阶段。

<p align="center">
  <img src="./docs/assets/readme/lead-structured-clarification-zh.png" alt="Another Atom 首次需求结构化澄清：Lead 提供平台、翻译引擎和语言范围选项，完成后进入下一步" width="640">
</p>

构建前，用户可以查看并修改 Product Manager 生成的产品说明，确认能力调整和范围后再进入后续流水线。

<p align="center">
  <img src="./docs/assets/readme/product-spec-approval-zh.png" alt="Another Atom 产品说明确认页：方案摘要、完整 Markdown 文档和确认构建入口" width="720">
</p>
...
构建完成后，Project 工作区继续保留与 Lead 的对话，并把真实 Preview、源码文件、设计产物、运行日志和发布入口放在同一界面；用户可以直接询问当前实现，也可以基于现有项目继续修改。

<p align="center">
  <img src="./docs/assets/readme/studio-completed-project-workspace-zh.png" alt="Another Atom 已完成项目工作区：连续对话、网页预览、源码与设计产物、运行日志和发布入口" width="960">
</p>

当 Lead 判断用户的要求需要修改代码时，会先整理修改目标、影响文件和保持不变的范围；只有用户点击“修改代码”后，系统才创建修改 Run 并写入代码。

<p align="center">
  <img src="./docs/assets/readme/project-change-proposal-zh.png" alt="Another Atom 项目修改确认：Lead 先说明修改范围，用户点击修改代码后才创建修改 Run" width="960">
</p>
...
发布由用户显式触发。发布成功后，工作区保留当前编辑状态，同时提供“打开公开应用”入口；后续生成或修改的新版本不会自动覆盖已发布版本。

<p align="center">
  <img src="./docs/assets/readme/project-publish-success-zh.png" alt="Another Atom 项目发布成功：工作区显示发布结果和打开公开应用入口" width="960">
</p>
...
- **[角色通过可检查产物协作]** 团队负责人（Lead）负责接收用户请求；产品经理（Product Manager）、架构师（Architect）和工程师（Engineer）依次交付产品规格（ProductSpec）、架构设计（ArchitectureDesign）、源码包（SourceBundle）与单元测试。构建产物（BuildArtifact）、执行报告（ExecutionReport）和校验报告（ValidationReport）由独立运行系统生成，不能由智能体自报通过。
...
- **[一次产品确认，风险变化重新确认]** 产品经理完成产品规格后，无论运行系统当前是否完整支持目标，都必须由用户确认一次；确认后的架构和工程阶段自动继续。如果架构结论要求改变已确认范围，或后续发生额外预算、破坏性源码变更或线上发布状态变化，系统必须重新获得对应确认。

### 3. 平台如何控制权限和执行

- **[模型负责生成，平台负责执行和授权]** LLM 负责理解需求、制定方案、生成代码和解释结果；平台 Runtime 负责身份、用量记录、流程状态、工具权限、仓库写入、Sandbox 和发布。模型可以提出要做什么，但不能绕过平台直接执行高权限操作。
