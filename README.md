...
这些原则分别约束产品对象、Agent 协作、权限边界和交付状态，避免把交互亮点与工程实现混在同一层讨论。

### 1. 产品围绕什么持续存在

- **[Project 是持续开发单元]** 用户得到的不是一次回答，而是一份持续存在的软件项目。需求、Agent 产物、代码仓库、Preview、版本和发布状态统一归入 Project；用户再次进入时，从已有代码和状态继续推进。

- **[所有编辑入口作用于同一份项目]** 对话用于表达意图，Preview 用于检查行为，视觉工具用于快速调整，源码文件用于精确控制。不同入口最终修改同一个 Project，并进入同一份版本历史。

### 2. Agent 如何协作

- **[角色用可检查产物交接]** Lead 负责入口；Product Manager、Architect、Engineer、Data Analyst、Reviewer 分别负责产品、架构、代码、数据和独立审查。角色价值由 Blueprint、ArchitectureSpec、源码、DataProfile、ValidationReport 和 ReviewReport 证明，不由头像数量或对话长度证明。

- **[Context 按当前任务最小化组装]** Agent 不共享一段无限增长的聊天记录。Runtime 只提供当前角色所需的 Context，并通过版本化 Artifact、Evidence 和 Handoff 传递结果，使输入、产出和失败原因能够检查、恢复和追溯。

- **[用户只在风险变化时介入]** 用户明确要求构建后，已授权范围和基础预算内的工作自动继续；范围变化、额外预算、破坏性代码操作和线上发布变化必须交还用户确认。

### 3. 权限和副作用由谁控制

- **[模型生成内容，Runtime 执行副作用]** LLM 负责理解、规划、生成和解释；Runtime 负责身份、配额、状态推进、工具权限、仓库写入、Sandbox 和发布。生成代码不能直接获得平台控制面的权限。

- **[所有私有入口遵循同一归属校验]** REST、SSE、私有 Preview 和 Terminal WebSocket 都通过统一 Gateway 识别 Session，并校验 Run、Project、Version 和 Sandbox Session 的用户归属。Public Route 单独建模，只读取用户明确发布的版本。

### 4. 交付如何归属、发布和恢复

- **[代码、Git 提交和版本可以相互追溯]** 每个 Project 默认拥有代码仓库。成功构建、用户编辑、代码保存和 Restore 形成 ProjectVersion 并映射 Git commit；自动修复的中间 Artifact 单独保留，只有通过校验的结果进入版本历史。

- **[工作版本与线上版本相互独立]** 生成完成不等于上线。工作版本可以持续变化，Public Route 只响应用户最后一次明确的 Publish/Update；Restore 创建新版本，不删除历史，也不自动改变发布指针。

- **[用户看到的状态来自持久化事实]** 阶段、错误、用量、产物和版本都必须对应可恢复状态。刷新、Worker 重启或恢复执行应复用已经完成的工作，避免重复调用、重复结算和重复创建版本。
